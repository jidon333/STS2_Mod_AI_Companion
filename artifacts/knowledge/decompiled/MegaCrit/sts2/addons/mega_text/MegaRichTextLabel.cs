...
public class MegaRichTextLabel : RichTextLabel
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = HasEffect(VariantUtils.ConvertTo<AbstractMegaRichTextEffect>(in args[0]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		bool from = AutoSizeEnabled;
		info.AddProperty(autoSizeEnabled, Variant.From(in from));
		StringName minFontSize = PropertyName.MinFontSize;
		int from2 = MinFontSize;
		info.AddProperty(minFontSize, Variant.From(in from2));
		StringName maxFontSize = PropertyName.MaxFontSize;
		from2 = MaxFontSize;
		info.AddProperty(maxFontSize, Variant.From(in from2));
		StringName isVerticallyBound = PropertyName.IsVerticallyBound;
		from = IsVerticallyBound;
		info.AddProperty(isVerticallyBound, Variant.From(in from));
		StringName isHorizontallyBound = PropertyName.IsHorizontallyBound;
		from = IsHorizontallyBound;
		info.AddProperty(isHorizontallyBound, Variant.From(in from));
		StringName text = PropertyName.Text;
		string from3 = Text;
		info.AddProperty(text, Variant.From(in from3));
