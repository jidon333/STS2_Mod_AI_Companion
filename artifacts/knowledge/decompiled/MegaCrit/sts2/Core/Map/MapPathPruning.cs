...
public static class MapPathPruning
{
	...
	{
		IReadOnlyList<MapPoint> segment2 = segment;
		return existingSegments.Any((MapPoint[] existingSegment) => OverlappingSegment(existingSegment, segment2));
	}
	...
	{
		MapPoint?[,] grid2 = grid;
		MapPoint[] segment2 = segment;
		bool result = false;
		for (int i = 0; i < segment2.Length - 1; i++)
		{
			MapPoint mapPoint = segment2[i];
			if (!IsInMap(grid2, mapPoint))
			{
				return true;
			}
			if (mapPoint.Children.Count > 1 || mapPoint.parents.Count > 1 || mapPoint.parents.Any((MapPoint n) => n.Children.Count == 1 && !IsRemoved(grid2, n)))
			{
				continue;
			}
			MapPoint[] source = segment2.Skip(i).ToArray();
			if (!source.Any((MapPoint n) => n.Children.Count > 1 && n.parents.Count == 1))
			{
				if (segment2[^1].parents.Count == 1)
				{
					return false;
				}
				if (!mapPoint.Children.Where((MapPoint c) => !segment2.Contains(c)).Any((MapPoint c) => c.parents.Count == 1))
				{
					RemovePoint(grid2, startMapPoints, mapPoint);
					result = true;
				}
			}
		}
