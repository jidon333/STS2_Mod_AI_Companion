...
public class RichTextAncientBanner : AbstractMegaRichTextEffect
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = _ProcessCustomFX(VariantUtils.ConvertTo<CharFXTransform>(in args[0]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		float from = Rotation;
		info.AddProperty(rotation, Variant.From(in from));
		StringName spacing = PropertyName.Spacing;
		from = Spacing;
		info.AddProperty(spacing, Variant.From(in from));
		StringName centerCharacter = PropertyName.CenterCharacter;
		from = CenterCharacter;
		info.AddProperty(centerCharacter, Variant.From(in from));
