...
public class LocManager
{
	...
	private CultureInfo CultureInfoFromThreeLetterCode(string language)
	{
		...
		try
		{
			CultureInfo cultureInfo = System.Globalization.CultureInfo.GetCultures(CultureTypes.NeutralCultures).FirstOrDefault((CultureInfo c) => c.ThreeLetterISOLanguageName == language2);
			if (cultureInfo != null)
			{
				return cultureInfo;
			}
		...
		string text = "Language code " + language2 + " could not be mapped to CultureInfo! Add a new manual mapping";
		Log.Error(text);
