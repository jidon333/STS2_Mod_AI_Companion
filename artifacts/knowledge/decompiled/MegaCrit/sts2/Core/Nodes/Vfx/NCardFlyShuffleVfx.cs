...
public class NCardFlyShuffleVfx : Control
{
	...
	private async Task PlayAnim()
	{
		...
		while (time2 / _duration <= 1f)
		{
			...
			c.Y -= _arcDir;
			base.GlobalPosition = MathHelper.BezierCurve(_startPos, _endPos, c, time2 / _duration);
			Vector2 vector = MathHelper.BezierCurve(_startPos, _endPos, c, (time2 + 0.05f) / _duration);
		...
		while (time2 / _duration <= 1f)
		{
			...
			base.Scale = Vector2.One * Mathf.Max(Mathf.Lerp(0.1f, -0.1f, time2 / _duration), 0f);
		}
