...
public class NCardSmithVfx : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NCardSmithVfx from = Create(VariantUtils.ConvertTo<NCard>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.Create && args.Count == 0)
		{
			NCardSmithVfx from2 = Create();
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		float from = SfxVolume;
		info.AddProperty(sfxVolume, Variant.From(in from));
