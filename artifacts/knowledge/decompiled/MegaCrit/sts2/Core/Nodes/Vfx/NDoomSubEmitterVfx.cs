...
public class NDoomSubEmitterVfx : Node2D
{
	...
	private void FireSpear(TextureRect textureRect = null)
	{
		...
		Vector2 position = textureRect2.Position;
		...
		textureRect2.Position = position;
		textureRect2.RotationDegrees = textureRect2.Position.X * _spearAngleIntensity + _rotationHackForSameDumbReason;
		textureRect2.Modulate = Colors.Transparent;
		float x = _spearFixedHScale / _curScaleX;
		textureRect2.Scale = new Vector2(x, Rng.Chaotic.NextFloat(_minSpearSize, _maxSpearSize));
		Vector2 vector = new Vector2(x, 0f);
		float num = Rng.Chaotic.NextFloat(_minSpearTime, _maxSpearTime);
		Tween tween = textureRect2.CreateTween();
		tween.TweenProperty(textureRect2, "scale", vector, num).From(new Vector2(x, Rng.Chaotic.NextFloat(_minSpearSize, _maxSpearSize)));
		if (_isOn)
		{
		...
		Tween tween2 = textureRect2.CreateTween();
		float a = Rng.Chaotic.NextFloat(0.4f, 0.7f);
		tween2.TweenProperty(textureRect2, "modulate", new Color(1f, 1f, 1f, a), 0.20000000298023224).SetEase(Tween.EaseType.In).SetTrans(Tween.TransitionType.Quad);
	}
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		float from = CurScaleX;
		info.AddProperty(curScaleX, Variant.From(in from));
