...
public class NBigSlashImpactVfx : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NBigSlashImpactVfx from = Create(VariantUtils.ConvertTo<Vector2>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.Create && args.Count == 3)
		{
			NBigSlashImpactVfx from2 = Create(VariantUtils.ConvertTo<Vector2>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]), VariantUtils.ConvertTo<Color>(in args[2]));
