...
public class NSpeechBubbleVfx : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NSpeechBubbleVfx from = Create(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<DialogueSide>(in args[1]), VariantUtils.ConvertTo<Vector2>(in args[2]), VariantUtils.ConvertTo<double>(in args[3]), VariantUtils.ConvertTo<VfxColor>(in args[4]));
		...
		{
			NSpeechBubbleVfx from2 = CreateInternal(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<DialogueSide>(in args[1]), VariantUtils.ConvertTo<double>(in args[2]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		double from = SecondsToDisplay;
		info.AddProperty(secondsToDisplay, Variant.From(in from));
