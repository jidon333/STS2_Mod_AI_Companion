...
public class NCardTrailVfx : Node2D
{
	...
	private void StopParticles(Tween tween)
	{
		...
		{
			if (child is CpuParticles2D @object)
			{
				tween.TweenProperty(@object, "amount", 1, 0.5);
			}
		}
	}
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NCardTrailVfx from = Create(VariantUtils.ConvertTo<Control>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
