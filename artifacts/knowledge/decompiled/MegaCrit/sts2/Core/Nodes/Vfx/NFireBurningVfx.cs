...
public class NFireBurningVfx : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NFireBurningVfx from = Create(VariantUtils.ConvertTo<Vector2>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]), VariantUtils.ConvertTo<bool>(in args[2]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.Create && args.Count == 4)
		{
			NFireBurningVfx from2 = Create(VariantUtils.ConvertTo<Vector2>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]), VariantUtils.ConvertTo<bool>(in args[2]), VariantUtils.ConvertTo<Color>(in args[3]));
