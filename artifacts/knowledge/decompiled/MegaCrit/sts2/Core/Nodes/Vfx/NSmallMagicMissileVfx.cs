...
public class NSmallMagicMissileVfx : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NSmallMagicMissileVfx from = Create(VariantUtils.ConvertTo<Vector2>(in args[0]), VariantUtils.ConvertTo<Color>(in args[1]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.GetProjectileDirection && args.Count == 0)
		{
			Vector2 from2 = GetProjectileDirection();
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.GetTopPosition && args.Count == 1)
		{
			Vector2 from3 = GetTopPosition(VariantUtils.ConvertTo<Vector2>(in args[0]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		float from = WaitTime;
		info.AddProperty(waitTime, Variant.From(in from));
