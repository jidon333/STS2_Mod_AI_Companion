...
public class NHitStop : Node
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Ease.Functions from = EaseForStrength(VariantUtils.ConvertTo<ShakeStrength>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.SecondsForDuration && args.Count == 1)
		{
			float from2 = SecondsForDuration(VariantUtils.ConvertTo<ShakeDuration>(in args[0]));
