...
public class NCombatUi : Control
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.PlayQueue)
		{
			NCardPlayQueue from8 = PlayQueue;
		...
		if (name == PropertyName.MessyCardPreviewContainer)
		{
			NMessyCardPreviewContainer from9 = MessyCardPreviewContainer;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Control from = EnergyCounterContainer;
		info.AddProperty(energyCounterContainer, Variant.From(in from));
		StringName endTurnButton = PropertyName.EndTurnButton;
		NEndTurnButton from2 = EndTurnButton;
		info.AddProperty(endTurnButton, Variant.From(in from2));
		StringName pingButton = PropertyName.PingButton;
		NPingButton from3 = PingButton;
		info.AddProperty(pingButton, Variant.From(in from3));
		StringName hand = PropertyName.Hand;
		NPlayerHand from4 = Hand;
		info.AddProperty(hand, Variant.From(in from4));
		StringName playContainer = PropertyName.PlayContainer;
		from = PlayContainer;
		info.AddProperty(playContainer, Variant.From(in from));
		StringName playQueue = PropertyName.PlayQueue;
		NCardPlayQueue from5 = PlayQueue;
		info.AddProperty(playQueue, Variant.From(in from5));
		StringName cardPreviewContainer = PropertyName.CardPreviewContainer;
		from = CardPreviewContainer;
		info.AddProperty(cardPreviewContainer, Variant.From(in from));
