...
public class NCreatureVisuals : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = IsPlayingHurtAnimation();
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Node2D from = Body;
		info.AddProperty(body, Variant.From(in from));
		StringName bounds = PropertyName.Bounds;
		Control from2 = Bounds;
		info.AddProperty(bounds, Variant.From(in from2));
		StringName intentPosition = PropertyName.IntentPosition;
		Marker2D from3 = IntentPosition;
		info.AddProperty(intentPosition, Variant.From(in from3));
		StringName orbPosition = PropertyName.OrbPosition;
		from3 = OrbPosition;
		info.AddProperty(orbPosition, Variant.From(in from3));
		StringName talkPosition = PropertyName.TalkPosition;
		from3 = TalkPosition;
		info.AddProperty(talkPosition, Variant.From(in from3));
		StringName vfxSpawnPosition = PropertyName.VfxSpawnPosition;
		from3 = VfxSpawnPosition;
		info.AddProperty(vfxSpawnPosition, Variant.From(in from3));
		StringName defaultScale = PropertyName.DefaultScale;
		float from4 = DefaultScale;
		info.AddProperty(defaultScale, Variant.From(in from4));
