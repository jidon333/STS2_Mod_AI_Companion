...
public class NCardPlayQueue : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Vector2 from = GetScaleForQueueIndex(VariantUtils.ConvertTo<int>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.GetPositionForQueueIndex && args.Count == 2)
		{
			Vector2 from2 = GetPositionForQueueIndex(VariantUtils.ConvertTo<NCard>(in args[0]), VariantUtils.ConvertTo<int>(in args[1]));
