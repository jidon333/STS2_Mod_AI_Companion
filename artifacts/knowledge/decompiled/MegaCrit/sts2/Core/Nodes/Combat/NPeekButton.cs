...
public class NPeekButton : NButton
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.IsPeeking)
		{
			bool from2 = IsPeeking;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.CurrentCardMarker)
		{
			Marker2D from3 = CurrentCardMarker;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		bool from = IsPeeking;
		info.AddProperty(isPeeking, Variant.From(in from));
		StringName currentCardMarker = PropertyName.CurrentCardMarker;
		Marker2D from2 = CurrentCardMarker;
		info.AddProperty(currentCardMarker, Variant.From(in from2));
