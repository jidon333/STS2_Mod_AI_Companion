...
public class NTargetManager : Node2D
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = AllowedToTargetNode(VariantUtils.ConvertTo<Node>(in args[0]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			bool from = IsInSelection;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.HoveredNode)
		{
			Node from2 = HoveredNode;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.LastTargetingFinishedFrame)
		{
			long from3 = LastTargetingFinishedFrame;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Node from = HoveredNode;
		info.AddProperty(hoveredNode, Variant.From(in from));
		StringName lastTargetingFinishedFrame = PropertyName.LastTargetingFinishedFrame;
		long from2 = LastTargetingFinishedFrame;
		info.AddProperty(lastTargetingFinishedFrame, Variant.From(in from2));
