...
public class NCreature : Control
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.PlayerIntentHandler)
		{
			NMultiplayerPlayerIntentHandler from7 = PlayerIntentHandler;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Control from = Hitbox;
		info.AddProperty(hitbox, Variant.From(in from));
		StringName orbManager = PropertyName.OrbManager;
		NOrbManager from2 = OrbManager;
		info.AddProperty(orbManager, Variant.From(in from2));
		StringName isInteractable = PropertyName.IsInteractable;
		bool from3 = IsInteractable;
		info.AddProperty(isInteractable, Variant.From(in from3));
		StringName visuals = PropertyName.Visuals;
		NCreatureVisuals from4 = Visuals;
		info.AddProperty(visuals, Variant.From(in from4));
		StringName intentContainer = PropertyName.IntentContainer;
		from = IntentContainer;
		info.AddProperty(intentContainer, Variant.From(in from));
		StringName isFocused = PropertyName.IsFocused;
		from3 = IsFocused;
		info.AddProperty(isFocused, Variant.From(in from3));
		StringName playerIntentHandler = PropertyName.PlayerIntentHandler;
		NMultiplayerPlayerIntentHandler from5 = PlayerIntentHandler;
		info.AddProperty(playerIntentHandler, Variant.From(in from5));
