...
public class NPlayerHand : Control
{
	...
	private void StartCardPlay(NHandCardHolder holder, bool startedViaShortcut)
	{
		...
		_draggedHolderIndex = holder2.GetIndex();
		_holdersAwaitingQueue.Add(holder2, _draggedHolderIndex);
		holder2.Reparent(this);
		holder2.BeginDrag();
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = IsAwaitingPlay(VariantUtils.ConvertTo<NHandCardHolder>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.Add && args.Count == 2)
		{
			NHandCardHolder from2 = Add(VariantUtils.ConvertTo<NCard>(in args[0]), VariantUtils.ConvertTo<int>(in args[1]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			from3 = IsInCardSelection;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.CurrentMode)
		{
			Mode from4 = CurrentMode;
			value = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (name == PropertyName.HasDraggedHolder)
		{
			from3 = HasDraggedHolder;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.FocusedHolder)
		{
			NHandCardHolder from5 = FocusedHolder;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Control from = CardHolderContainer;
		info.AddProperty(cardHolderContainer, Variant.From(in from));
		StringName peekButton = PropertyName.PeekButton;
		NPeekButton from2 = PeekButton;
		info.AddProperty(peekButton, Variant.From(in from2));
		StringName currentMode = PropertyName.CurrentMode;
		Mode from3 = CurrentMode;
		info.AddProperty(currentMode, Variant.From(in from3));
		StringName focusedHolder = PropertyName.FocusedHolder;
		NHandCardHolder from4 = FocusedHolder;
		info.AddProperty(focusedHolder, Variant.From(in from4));
