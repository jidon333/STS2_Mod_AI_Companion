...
public class NHealthBar : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			bool from = IsPoisonLethal(VariantUtils.ConvertTo<int>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.IsDoomLethal && args.Count == 2)
		{
			bool from2 = IsDoomLethal(VariantUtils.ConvertTo<int>(in args[0]), VariantUtils.ConvertTo<int>(in args[1]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.GetFgWidth && args.Count == 1)
		{
			float from3 = GetFgWidth(VariantUtils.ConvertTo<int>(in args[0]));
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (method == MethodName.GetFgWidth && args.Count == 2)
		{
			float from4 = GetFgWidth(VariantUtils.ConvertTo<int>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			float from = MaxFgWidth;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.HpBarContainer)
		{
			Control from2 = HpBarContainer;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		Control from = HpBarContainer;
		info.AddProperty(hpBarContainer, Variant.From(in from));
