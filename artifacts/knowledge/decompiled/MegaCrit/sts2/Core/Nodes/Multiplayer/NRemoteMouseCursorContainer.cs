...
public class NRemoteMouseCursorContainer : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Vector2 from = GetCursorPosition(VariantUtils.ConvertTo<ulong>(in args[0]));
		...
		{
			DrawingMode from2 = GetDrawingMode(VariantUtils.ConvertTo<ulong>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.GetCursor && args.Count == 1)
		{
			NRemoteMouseCursor from3 = GetCursor(VariantUtils.ConvertTo<ulong>(in args[0]));
	...
	internal static bool InvokeGodotClassStaticMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		if (method == MethodName.GetDrawingMode && args.Count == 1)
		{
			DrawingMode from = GetDrawingMode(VariantUtils.ConvertTo<ulong>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		ret = default(godot_variant);
