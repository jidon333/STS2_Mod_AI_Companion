...
public class NRemoteMouseCursor : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NRemoteMouseCursor from = Create(VariantUtils.ConvertTo<ulong>(in args[0]));
		...
		{
			Vector2 from2 = GetHotspot(VariantUtils.ConvertTo<DrawingMode>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.GetTexture && args.Count == 2)
		{
			Texture2D from3 = GetTexture(VariantUtils.ConvertTo<bool>(in args[0]), VariantUtils.ConvertTo<DrawingMode>(in args[1]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		ulong from = PlayerId;
		info.AddProperty(playerId, Variant.From(in from));
