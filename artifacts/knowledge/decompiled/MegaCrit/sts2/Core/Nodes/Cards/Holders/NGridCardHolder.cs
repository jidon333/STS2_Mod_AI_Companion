...
public class NGridCardHolder : NCardHolder, IPoolable
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NGridCardHolder from = Create(VariantUtils.ConvertTo<NCard>(in args[0]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.CardLibraryStats)
		{
			NCardLibraryStats from = CardLibraryStats;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.IsShowingUpgradedCard)
		{
			bool from2 = IsShowingUpgradedCard;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NCardLibraryStats from = CardLibraryStats;
		info.AddProperty(cardLibraryStats, Variant.From(in from));
