...
public class NCardGrid : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Vector2 from2 = GetContainedCardsSize();
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		int from = DisplayedRows;
		info.AddProperty(displayedRows, Variant.From(in from));
		StringName isShowingUpgrades = PropertyName.IsShowingUpgrades;
		bool from2 = IsShowingUpgrades;
		info.AddProperty(isShowingUpgrades, Variant.From(in from2));
		StringName yOffset = PropertyName.YOffset;
		from = YOffset;
		info.AddProperty(yOffset, Variant.From(in from));
