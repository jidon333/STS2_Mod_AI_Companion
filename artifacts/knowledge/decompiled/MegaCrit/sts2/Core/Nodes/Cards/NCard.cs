...
public class NCard : Control, IPoolable
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Color from2 = GetCostTextColorInHand(VariantUtils.ConvertTo<CardCostColor>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]), VariantUtils.ConvertTo<Color>(in args[2]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.GetCostOutlineColorInHand && args.Count == 3)
		{
			Color from3 = GetCostOutlineColorInHand(VariantUtils.ConvertTo<CardCostColor>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]), VariantUtils.ConvertTo<Color>(in args[2]));
		...
		{
			Color from4 = GetTitleLabelOutlineColor();
	...
	internal static bool InvokeGodotClassStaticMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		if (method == MethodName.GetCostTextColorInHand && args.Count == 3)
		{
			Color from = GetCostTextColorInHand(VariantUtils.ConvertTo<CardCostColor>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]), VariantUtils.ConvertTo<Color>(in args[2]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.GetCostOutlineColorInHand && args.Count == 3)
		{
			Color from2 = GetCostOutlineColorInHand(VariantUtils.ConvertTo<CardCostColor>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]), VariantUtils.ConvertTo<Color>(in args[2]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		ret = default(godot_variant);
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.CardHighlight)
		{
			NCardHighlight from = CardHighlight;
		...
		if (name == PropertyName.Visibility)
		{
			ModelVisibility from3 = Visibility;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NCardHighlight from = CardHighlight;
		info.AddProperty(cardHighlight, Variant.From(in from));
		StringName body = PropertyName.Body;
		Control from2 = Body;
		info.AddProperty(body, Variant.From(in from2));
		StringName visibility = PropertyName.Visibility;
		ModelVisibility from3 = Visibility;
		info.AddProperty(visibility, Variant.From(in from3));
		StringName playPileTween = PropertyName.PlayPileTween;
		Tween from4 = PlayPileTween;
		info.AddProperty(playPileTween, Variant.From(in from4));
		StringName randomizeCostTween = PropertyName.RandomizeCostTween;
		from4 = RandomizeCostTween;
		info.AddProperty(randomizeCostTween, Variant.From(in from4));
		StringName displayingPile = PropertyName.DisplayingPile;
		PileType from5 = DisplayingPile;
		info.AddProperty(displayingPile, Variant.From(in from5));
