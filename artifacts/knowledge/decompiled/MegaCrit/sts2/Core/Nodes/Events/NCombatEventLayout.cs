...
public class NCombatEventLayout : NEventLayout
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.EmbeddedCombatRoom)
		{
			NCombatRoom from = EmbeddedCombatRoom;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.HasCombatStarted)
		{
			bool from2 = HasCombatStarted;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NCombatRoom from = EmbeddedCombatRoom;
		info.AddProperty(embeddedCombatRoom, Variant.From(in from));
		StringName hasCombatStarted = PropertyName.HasCombatStarted;
		bool from2 = HasCombatStarted;
		info.AddProperty(hasCombatStarted, Variant.From(in from2));
