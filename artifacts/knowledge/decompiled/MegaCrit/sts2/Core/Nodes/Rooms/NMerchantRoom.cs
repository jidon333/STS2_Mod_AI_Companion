...
public class NMerchantRoom : Control, IScreenContext, IRoomWithProceedButton
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			NProceedButton from = ProceedButton;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.Inventory)
		{
			NMerchantInventory from2 = Inventory;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.MerchantButton)
		{
			NMerchantButton from3 = MerchantButton;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NMerchantInventory from = Inventory;
		info.AddProperty(inventory, Variant.From(in from));
		StringName merchantButton = PropertyName.MerchantButton;
		NMerchantButton from2 = MerchantButton;
		info.AddProperty(merchantButton, Variant.From(in from2));
