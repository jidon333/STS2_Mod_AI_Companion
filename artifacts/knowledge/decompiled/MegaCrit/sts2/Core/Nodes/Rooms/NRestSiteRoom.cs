...
public class NRestSiteRoom : Control, IScreenContext, IRoomWithProceedButton
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.Header)
		{
			MegaLabel from2 = Header;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.Description)
		{
			MegaRichTextLabel from3 = Description;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		MegaLabel from = Header;
		info.AddProperty(header, Variant.From(in from));
		StringName description = PropertyName.Description;
		MegaRichTextLabel from2 = Description;
		info.AddProperty(description, Variant.From(in from2));
		StringName bgContainer = PropertyName.BgContainer;
		Control from3 = BgContainer;
		info.AddProperty(bgContainer, Variant.From(in from3));
