...
public class NHoverTipSet : Control
{
	...
	public static NHoverTipSet CreateAndShow(Control owner, IEnumerable<IHoverTip> hoverTips, HoverTipAlignment alignment = HoverTipAlignment.None)
	{
		...
		owner2.Connect(Node.SignalName.TreeExiting, Callable.From(delegate
		{
			Remove(owner2);
		}));
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NHoverTipSet from = CreateAndShowMapPointHistory(VariantUtils.ConvertTo<Control>(in args[0]), VariantUtils.ConvertTo<NMapPointHistoryHoverTip>(in args[1]));
