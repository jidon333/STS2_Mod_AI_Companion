...
public class NReaction : TextureRect
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NReaction from = Create(VariantUtils.ConvertTo<Texture2D>(in args[0]));
		...
		{
			Texture2D from2 = TypeToTexture(VariantUtils.ConvertTo<ReactionType>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.TextureToType && args.Count == 1)
		{
			ReactionType from3 = TextureToType(VariantUtils.ConvertTo<Texture2D>(in args[0]));
