...
public class NPatchNotesScreen : Control, IScreenContext
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			string from = ReadPatchNoteFile(VariantUtils.ConvertTo<string>(in args[0]));
		...
		{
			string from2 = GetFileNameFromPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.RemoveFileExtension && args.Count == 1)
		{
			string from3 = RemoveFileExtension(VariantUtils.ConvertTo<string>(in args[0]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			Control from2 = DefaultFocusedControl;
