...
public class NMapPointHistoryEntry : NClickableControl
{
	...
	public async Task AnimateIn(int index)
	{
		...
		float from = ((SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.4f : 0.5f);
		float to = 0.2f;
		int num = ((SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 5 : 9);
		float weight = (float)index / (float)(index + num);
		float seconds = Mathf.Lerp(from, to, weight);
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		int from = FloorNum;
		info.AddProperty(floorNum, Variant.From(in from));
