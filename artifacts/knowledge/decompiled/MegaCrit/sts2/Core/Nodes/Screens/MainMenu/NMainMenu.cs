...
public class NMainMenu : Control, IScreenContext
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NMainMenu from = Create(VariantUtils.ConvertTo<bool>(in args[0]));
		...
		{
			NSingleplayerSubmenu from2 = OpenSingleplayerSubmenu();
		...
		{
			NMultiplayerSubmenu from3 = OpenMultiplayerSubmenu();
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.PatchNotesScreen)
		{
			NPatchNotesScreen from = PatchNotesScreen;
		...
		if (name == PropertyName.SubmenuStack)
		{
			NMainMenuSubmenuStack from3 = SubmenuStack;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.ContinueRunInfo)
		{
			NContinueRunInfo from4 = ContinueRunInfo;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NPatchNotesScreen from = PatchNotesScreen;
		info.AddProperty(patchNotesScreen, Variant.From(in from));
		StringName blurBackstop = PropertyName.BlurBackstop;
		Control from2 = BlurBackstop;
		info.AddProperty(blurBackstop, Variant.From(in from2));
		StringName submenuStack = PropertyName.SubmenuStack;
		NMainMenuSubmenuStack from3 = SubmenuStack;
		info.AddProperty(submenuStack, Variant.From(in from3));
