...
public class NHandImage : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Tween from = DoFightMove(VariantUtils.ConvertTo<RelicPickingFightMove>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		int from = Index;
		info.AddProperty(index, Variant.From(in from));
		StringName isDown = PropertyName.IsDown;
		bool from2 = IsDown;
		info.AddProperty(isDown, Variant.From(in from2));
