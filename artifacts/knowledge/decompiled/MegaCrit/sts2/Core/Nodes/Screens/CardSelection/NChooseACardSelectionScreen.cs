...
public class NChooseACardSelectionScreen : Control, IOverlayScreen, IScreenContext, ICardSelector
{
	...
	public override void _Ready()
	{
		...
		for (int j = 0; j < _cardRow.GetChildCount(); j++)
		{
			Control child = _cardRow.GetChild<Control>(j);
			child.FocusNeighborBottom = child.GetPath();
			child.FocusNeighborTop = child.GetPath();
			child.FocusNeighborLeft = ((j > 0) ? _cardRow.GetChild(j - 1).GetPath() : _cardRow.GetChild(_cardRow.GetChildCount() - 1).GetPath());
			child.FocusNeighborRight = ((j < _cardRow.GetChildCount() - 1) ? _cardRow.GetChild(j + 1).GetPath() : _cardRow.GetChild(0).GetPath());
		}
