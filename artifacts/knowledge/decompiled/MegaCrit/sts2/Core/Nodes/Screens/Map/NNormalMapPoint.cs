...
public class NNormalMapPoint : NMapPoint
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			string from = IconName(VariantUtils.ConvertTo<MapPointType>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.IconPath && args.Count == 1)
		{
			string from2 = IconPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.OutlinePath && args.Count == 1)
		{
			string from3 = OutlinePath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (method == MethodName.UnknownIconPath && args.Count == 1)
		{
			string from4 = UnknownIconPath(VariantUtils.ConvertTo<RoomType>(in args[0]));
			ret = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (method == MethodName.UnknownOutlinePath && args.Count == 1)
		{
			string from5 = UnknownOutlinePath(VariantUtils.ConvertTo<RoomType>(in args[0]));
