...
public class NGameOverScreen : NClickableControl, IOverlayScreen, IScreenContext
{
	...
	private async Task AnimateScoreBar()
	{
		...
		if (unlocksRemaining > 0)
		{
			...
			Tween scoreTween5 = CreateTween();
			scoreTween5.TweenProperty(_scoreBar, "modulate:a", 1f, 0.3);
			await scoreTween5.ToSignal(scoreTween5, Tween.SignalName.Finished);
			if (currentScore3 + _score >= _scoreThreshold)
			{
				...
				scoreTween5 = CreateTween().SetParallel();
				scoreTween5.TweenInterval(1.0);
				scoreTween5.Chain();
				scoreTween5.TweenMethod(Callable.From<int>(TweenScore), currentScore3 + _scoreThreshold, _scoreThreshold, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic);
				scoreTween5.TweenProperty(_scoreFg, "scale:x", 1f, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic);
				scoreTween5.Chain();
				scoreTween5.TweenCallback(Callable.From(PlayUnlockSfx));
				scoreTween5.TweenProperty(node, "modulate:a", 1f, 0.25);
				scoreTween5.TweenProperty(node, "position:y", -60f, 0.25).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Spring);
				await scoreTween5.ToSignal(scoreTween5, Tween.SignalName.Finished);
				...
				else if (currentScore3 >= newThreshold)
				{
					...
					scoreTween5.Chain();
					scoreTween5.TweenMethod(Callable.From<int>(TweenScore), 0, newThreshold * 99 / 100, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic);
					scoreTween5.TweenProperty(_scoreFg, "scale:x", 1f, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic)
						.From(0f);
					await scoreTween5.ToSignal(scoreTween5, Tween.SignalName.Finished);
				...
				else
				{
					...
					scoreTween5.TweenInterval(0.5);
					scoreTween5.Chain();
					scoreTween5.TweenMethod(Callable.From<int>(TweenScore), 0, currentScore3, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic);
					scoreTween5.TweenProperty(_scoreFg, "scale:x", (float)currentScore3 / (float)newThreshold, 1.0).SetEase(Tween.EaseType.Out).SetTrans(Tween.TransitionType.Cubic)
						.From(0f);
					await scoreTween5.ToSignal(scoreTween5, Tween.SignalName.Finished);
			...
			else
			{
				...
				scoreTween5 = CreateTween().SetParallel();
				scoreTween5.TweenInterval(0.5);
				scoreTween5.TweenMethod(Callable.From<int>(TweenScore), currentScore3, currentScore3 + _score, 1.0);
				scoreTween5.TweenProperty(_scoreFg, "scale:x", (float)(currentScore3 + _score) / (float)_scoreThreshold, 1.0);
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			int from2 = GetScoreThreshold(VariantUtils.ConvertTo<int>(in args[0]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		{
			NetScreenType from = ScreenType;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.UseSharedBackstop)
		{
			bool from2 = UseSharedBackstop;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.DefaultFocusedControl)
		{
			Control from3 = DefaultFocusedControl;
