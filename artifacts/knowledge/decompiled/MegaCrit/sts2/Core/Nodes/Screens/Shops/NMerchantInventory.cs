...
public class NMerchantInventory : Control, IScreenContext
{
	...
	private void UpdateHorizontalNavigation()
	{
		...
		{
			list4[i].FocusNeighborLeft = ((i > 0) ? list4[i - 1].GetPath() : list4[i].GetPath());
			list4[i].FocusNeighborRight = ((i < list4.Count - 1) ? list4[i + 1].GetPath() : list4[i].GetPath());
		}
		for (int j = 0; j < list6.Count; j++)
		{
			list6[j].FocusNeighborLeft = ((j > 0) ? list6[j - 1].GetPath() : list6[j].GetPath());
			list6[j].FocusNeighborRight = ((j < list6.Count - 1) ? list6[j + 1].GetPath() : list6[j].GetPath());
		}
		for (int k = 0; k < list5.Count; k++)
		{
			list5[k].FocusNeighborLeft = ((k > 0) ? list5[k - 1].GetPath() : list5[k].GetPath());
			list5[k].FocusNeighborRight = ((k < list5.Count - 1) ? list5[k + 1].GetPath() : list5[k].GetPath());
		}
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.MerchantHand)
		{
			NMerchantHand from2 = MerchantHand;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NMerchantHand from2 = MerchantHand;
		info.AddProperty(merchantHand, Variant.From(in from2));
