...
public abstract class NMapDrawingInput : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NMapDrawingInput from = Create(VariantUtils.ConvertTo<NMapDrawings>(in args[0]), VariantUtils.ConvertTo<DrawingMode>(in args[1]), VariantUtils.ConvertTo<bool>(in args[2]));
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		DrawingMode from = DrawingMode;
		info.AddProperty(drawingMode, Variant.From(in from));
