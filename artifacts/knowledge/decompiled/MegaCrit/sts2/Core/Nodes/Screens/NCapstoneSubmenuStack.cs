...
public class NCapstoneSubmenuStack : Control, ICapstoneScreen, IScreenContext
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NSubmenu from = ShowScreen(VariantUtils.ConvertTo<CapstoneSubmenuType>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.GetCapstoneSubmenuType && args.Count == 0)
		{
			NetScreenType from2 = GetCapstoneSubmenuType();
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.Type)
		{
			CapstoneSubmenuType from = Type;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.Stack)
		{
			NRunSubmenuStack from2 = Stack;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.ScreenType)
		{
			NetScreenType from3 = ScreenType;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.UseSharedBackstop)
		{
			bool from4 = UseSharedBackstop;
			value = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (name == PropertyName.DefaultFocusedControl)
		{
			Control from5 = DefaultFocusedControl;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		CapstoneSubmenuType from = Type;
		info.AddProperty(type, Variant.From(in from));
		StringName stack = PropertyName.Stack;
		NRunSubmenuStack from2 = Stack;
		info.AddProperty(stack, Variant.From(in from2));
	}
