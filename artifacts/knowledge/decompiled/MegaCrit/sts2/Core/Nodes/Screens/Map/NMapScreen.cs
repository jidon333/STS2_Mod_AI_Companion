...
public class NMapScreen : Control, IScreenContext, INetCursorPositionTranslator
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			Vector2 from = GetLineEndpoint(VariantUtils.ConvertTo<NMapPoint>(in args[0]));
		...
		{
			NMapScreen from3 = Open(VariantUtils.ConvertTo<bool>(in args[0]));
		...
		{
			Vector2 from4 = GetNetPositionFromScreenPosition(VariantUtils.ConvertTo<Vector2>(in args[0]));
			ret = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (method == MethodName.GetMapPositionFromNetPosition && args.Count == 1)
		{
			Vector2 from5 = GetMapPositionFromNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]));
			ret = VariantUtils.CreateFrom(in from5);
			return true;
		}
		if (method == MethodName.GetScreenPositionFromNetPosition && args.Count == 1)
		{
			Vector2 from6 = GetScreenPositionFromNetPosition(VariantUtils.ConvertTo<Vector2>(in args[0]));
			ret = VariantUtils.CreateFrom(in from6);
			return true;
		}
		if (method == MethodName.IsNodeOnScreen && args.Count == 1)
		{
			bool from7 = IsNodeOnScreen(VariantUtils.ConvertTo<NMapPoint>(in args[0]));
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.Drawings)
		{
			NMapDrawings from3 = Drawings;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.DefaultFocusedControl)
		{
			Control from4 = DefaultFocusedControl;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		bool from = IsOpen;
		info.AddProperty(isOpen, Variant.From(in from));
		StringName isTravelEnabled = PropertyName.IsTravelEnabled;
		from = IsTravelEnabled;
		info.AddProperty(isTravelEnabled, Variant.From(in from));
		StringName isDebugTravelEnabled = PropertyName.IsDebugTravelEnabled;
		from = IsDebugTravelEnabled;
		info.AddProperty(isDebugTravelEnabled, Variant.From(in from));
		StringName isTraveling = PropertyName.IsTraveling;
		from = IsTraveling;
		info.AddProperty(isTraveling, Variant.From(in from));
		StringName drawings = PropertyName.Drawings;
		NMapDrawings from2 = Drawings;
		info.AddProperty(drawings, Variant.From(in from2));
