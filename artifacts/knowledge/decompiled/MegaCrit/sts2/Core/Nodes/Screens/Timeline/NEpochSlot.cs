...
public class NEpochSlot : NButton
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.State)
		{
			EpochSlotState from2 = State;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.HasSpawned)
		{
			bool from3 = HasSpawned;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		EpochSlotState from = State;
		info.AddProperty(state, Variant.From(in from));
		StringName hasSpawned = PropertyName.HasSpawned;
		bool from2 = HasSpawned;
		info.AddProperty(hasSpawned, Variant.From(in from2));
