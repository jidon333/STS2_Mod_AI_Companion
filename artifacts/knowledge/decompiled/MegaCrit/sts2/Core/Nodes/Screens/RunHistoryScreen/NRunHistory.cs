...
public class NRunHistory : NSubmenu
{
	...
	private void LoadGoldHpAndPotionInfo(NRunHistoryPlayerIcon icon)
	{
		...
		{
			CharacterModel byId = ModelDb.GetById<CharacterModel>(icon2.Player.Character);
		...
		UnlockState unlockState = SaveManager.Instance.GenerateUnlockStateFromProgress();
		Player owner = Player.CreateForNewRun(ModelDb.GetById<CharacterModel>(icon2.Player.Character), unlockState, icon2.Player.Id);
