...
public abstract class NMapPoint : NButton
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		...
		if (name == PropertyName.VoteContainer)
		{
			NMultiplayerVoteContainer from4 = VoteContainer;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NMultiplayerVoteContainer from = VoteContainer;
		info.AddProperty(voteContainer, Variant.From(in from));
		StringName state = PropertyName.State;
		MapPointState from2 = State;
		info.AddProperty(state, Variant.From(in from2));
