...
public class NVerticalPopup : Control
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.TitleLabel)
		{
			MegaLabel from = TitleLabel;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.BodyLabel)
		{
			MegaRichTextLabel from2 = BodyLabel;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		NPopupYesNoButton from3;
		if (name == PropertyName.YesButton)
		{
			from3 = YesButton;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		MegaLabel from = TitleLabel;
		info.AddProperty(titleLabel, Variant.From(in from));
		StringName bodyLabel = PropertyName.BodyLabel;
		MegaRichTextLabel from2 = BodyLabel;
		info.AddProperty(bodyLabel, Variant.From(in from2));
		StringName yesButton = PropertyName.YesButton;
		NPopupYesNoButton from3 = YesButton;
		info.AddProperty(yesButton, Variant.From(in from3));
		StringName noButton = PropertyName.NoButton;
		from3 = NoButton;
		info.AddProperty(noButton, Variant.From(in from3));
	}
