...
public class NSearchBar : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			string from = Normalize(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.RemoveHtmlTags && args.Count == 1)
		{
			string from2 = RemoveHtmlTags(VariantUtils.ConvertTo<string>(in args[0]));
