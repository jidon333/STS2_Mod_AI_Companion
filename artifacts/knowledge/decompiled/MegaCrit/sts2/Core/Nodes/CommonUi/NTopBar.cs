...
public class NTopBar : Control
{
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.Map)
		{
			NTopBarMapButton from = Map;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.Deck)
		{
			NTopBarDeckButton from2 = Deck;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.Pause)
		{
			NTopBarPauseButton from3 = Pause;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.PotionContainer)
		{
			NPotionContainer from4 = PotionContainer;
			value = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (name == PropertyName.RoomIcon)
		{
			NTopBarRoomIcon from5 = RoomIcon;
			value = VariantUtils.CreateFrom(in from5);
			return true;
		}
		if (name == PropertyName.FloorIcon)
		{
			NTopBarFloorIcon from6 = FloorIcon;
			value = VariantUtils.CreateFrom(in from6);
			return true;
		}
		if (name == PropertyName.BossIcon)
		{
			NTopBarBossIcon from7 = BossIcon;
			value = VariantUtils.CreateFrom(in from7);
			return true;
		}
		if (name == PropertyName.Gold)
		{
			NTopBarGold from8 = Gold;
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NTopBarMapButton from = Map;
		info.AddProperty(map, Variant.From(in from));
		StringName deck = PropertyName.Deck;
		NTopBarDeckButton from2 = Deck;
		info.AddProperty(deck, Variant.From(in from2));
		StringName pause = PropertyName.Pause;
		NTopBarPauseButton from3 = Pause;
		info.AddProperty(pause, Variant.From(in from3));
		StringName potionContainer = PropertyName.PotionContainer;
		NPotionContainer from4 = PotionContainer;
		info.AddProperty(potionContainer, Variant.From(in from4));
		StringName roomIcon = PropertyName.RoomIcon;
		NTopBarRoomIcon from5 = RoomIcon;
		info.AddProperty(roomIcon, Variant.From(in from5));
		StringName floorIcon = PropertyName.FloorIcon;
		NTopBarFloorIcon from6 = FloorIcon;
		info.AddProperty(floorIcon, Variant.From(in from6));
		StringName bossIcon = PropertyName.BossIcon;
		NTopBarBossIcon from7 = BossIcon;
		info.AddProperty(bossIcon, Variant.From(in from7));
		StringName gold = PropertyName.Gold;
		NTopBarGold from8 = Gold;
		info.AddProperty(gold, Variant.From(in from8));
		StringName hp = PropertyName.Hp;
		NTopBarHp from9 = Hp;
		info.AddProperty(hp, Variant.From(in from9));
		StringName portrait = PropertyName.Portrait;
		NTopBarPortrait from10 = Portrait;
		info.AddProperty(portrait, Variant.From(in from10));
		StringName portraitTip = PropertyName.PortraitTip;
		NTopBarPortraitTip from11 = PortraitTip;
		info.AddProperty(portraitTip, Variant.From(in from11));
		StringName timer = PropertyName.Timer;
		NRunTimer from12 = Timer;
		info.AddProperty(timer, Variant.From(in from12));
		StringName trailContainer = PropertyName.TrailContainer;
		Node from13 = TrailContainer;
		info.AddProperty(trailContainer, Variant.From(in from13));
