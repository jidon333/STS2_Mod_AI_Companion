...
public class NMultiplayerVoteContainer : Control
{
	...
	private void AnimVoteIn(VoteIcon vote, bool animate)
	{
		...
		{
			vote2.tween?.Kill();
			vote2.tween = CreateTween().SetParallel();
			vote2.tween.TweenProperty(vote2.node, "modulate:a", 1f, 0.2).From(0f);
			vote2.tween.TweenProperty(vote2.node, "position:y", 0f, 0.3).From(20f).SetTrans(Tween.TransitionType.Back)
	...
	private void AnimVoteOut(VoteIcon vote, bool animate)
	{
		...
		if (animate)
		{
			vote2.tween?.Kill();
			vote2.tween = CreateTween().SetParallel();
			vote2.tween.TweenProperty(vote2.node, "modulate:a", 0f, 0.1).SetDelay(0.15000000596046448);
			vote2.tween.TweenProperty(vote2.node, "position:y", 20f, 0.25).SetTrans(Tween.TransitionType.Expo).SetEase(Tween.EaseType.In);
			vote2.tween.Chain().TweenCallback(Callable.From(delegate
			{
