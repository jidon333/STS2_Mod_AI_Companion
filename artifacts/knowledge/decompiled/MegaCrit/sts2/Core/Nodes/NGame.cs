...
public class NGame : Control
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			NInspectRelicScreen from3 = GetInspectRelicScreen();
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (method == MethodName.GetInspectCardScreen && args.Count == 0)
		{
			NInspectCardScreen from4 = GetInspectCardScreen();
		...
		{
			WorldEnvironment from5 = ActivateWorldEnvironment();
		...
		{
			string from6 = FormatBytes(VariantUtils.ConvertTo<ulong>(in args[0]));
	...
	internal static bool InvokeGodotClassStaticMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		if (method == MethodName.FormatBytes && args.Count == 1)
		{
			string from3 = FormatBytes(VariantUtils.ConvertTo<ulong>(in args[0]));
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		ret = default(godot_variant);
	...
	protected override bool GetGodotClassPropertyValue(in godot_string_name name, out godot_variant value)
	{
		if (name == PropertyName.RootSceneContainer)
		{
			NSceneContainer from = RootSceneContainer;
			value = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (name == PropertyName.HoverTipsContainer)
		{
			Node from2 = HoverTipsContainer;
			value = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (name == PropertyName.MainMenu)
		{
			NMainMenu from3 = MainMenu;
			value = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (name == PropertyName.CurrentRunNode)
		{
			NRun from4 = CurrentRunNode;
			value = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (name == PropertyName.LogoAnimation)
		{
			NLogoAnimation from5 = LogoAnimation;
			value = VariantUtils.CreateFrom(in from5);
			return true;
		}
		if (name == PropertyName.Transition)
		{
			NTransition from6 = Transition;
			value = VariantUtils.CreateFrom(in from6);
			return true;
		}
		if (name == PropertyName.TimeoutOverlay)
		{
			NMultiplayerTimeoutOverlay from7 = TimeoutOverlay;
			value = VariantUtils.CreateFrom(in from7);
			return true;
		}
		if (name == PropertyName.AudioManager)
		{
			NAudioManager from8 = AudioManager;
			value = VariantUtils.CreateFrom(in from8);
			return true;
		}
		if (name == PropertyName.RemoteCursorContainer)
		{
			NRemoteMouseCursorContainer from9 = RemoteCursorContainer;
			value = VariantUtils.CreateFrom(in from9);
			return true;
		}
		if (name == PropertyName.InputManager)
		{
			NInputManager from10 = InputManager;
			value = VariantUtils.CreateFrom(in from10);
			return true;
		}
		if (name == PropertyName.HotkeyManager)
		{
			NHotkeyManager from11 = HotkeyManager;
			value = VariantUtils.CreateFrom(in from11);
			return true;
		}
		if (name == PropertyName.ReactionWheel)
		{
			NReactionWheel from12 = ReactionWheel;
			value = VariantUtils.CreateFrom(in from12);
			return true;
		}
		if (name == PropertyName.ReactionContainer)
		{
			NReactionContainer from13 = ReactionContainer;
			value = VariantUtils.CreateFrom(in from13);
			return true;
		}
		if (name == PropertyName.CursorManager)
		{
			NCursorManager from14 = CursorManager;
			value = VariantUtils.CreateFrom(in from14);
			return true;
		}
		if (name == PropertyName.DebugAudio)
		{
			NDebugAudioManager from15 = DebugAudio;
			value = VariantUtils.CreateFrom(in from15);
			return true;
		}
		if (name == PropertyName.DebugSeedOverride)
		{
			string from16 = DebugSeedOverride;
			value = VariantUtils.CreateFrom(in from16);
			return true;
		}
		if (name == PropertyName.StartOnMainMenu)
		{
			bool from17 = StartOnMainMenu;
			value = VariantUtils.CreateFrom(in from17);
			return true;
		}
		if (name == PropertyName.InspectRelicScreen)
		{
			NInspectRelicScreen from18 = InspectRelicScreen;
			value = VariantUtils.CreateFrom(in from18);
			return true;
		}
		if (name == PropertyName.InspectCardScreen)
		{
			NInspectCardScreen from19 = InspectCardScreen;
			value = VariantUtils.CreateFrom(in from19);
			return true;
		}
		if (name == PropertyName.FeedbackScreen)
		{
			NSendFeedbackScreen from20 = FeedbackScreen;
			value = VariantUtils.CreateFrom(in from20);
			return true;
		}
		if (name == PropertyName.WorldEnvironment)
		{
			WorldEnvironment from21 = WorldEnvironment;
			value = VariantUtils.CreateFrom(in from21);
			return true;
		}
		if (name == PropertyName.HitStop)
		{
			NHitStop from22 = HitStop;
			value = VariantUtils.CreateFrom(in from22);
			return true;
		}
		if (name == PropertyName._inspectionContainer)
		{
	...
	protected override void SaveGodotObjectData(GodotSerializationInfo info)
	{
		...
		NSceneContainer from = RootSceneContainer;
		info.AddProperty(rootSceneContainer, Variant.From(in from));
		StringName hoverTipsContainer = PropertyName.HoverTipsContainer;
		Node from2 = HoverTipsContainer;
		info.AddProperty(hoverTipsContainer, Variant.From(in from2));
		StringName transition = PropertyName.Transition;
		NTransition from3 = Transition;
		info.AddProperty(transition, Variant.From(in from3));
		StringName timeoutOverlay = PropertyName.TimeoutOverlay;
		NMultiplayerTimeoutOverlay from4 = TimeoutOverlay;
		info.AddProperty(timeoutOverlay, Variant.From(in from4));
		StringName audioManager = PropertyName.AudioManager;
		NAudioManager from5 = AudioManager;
		info.AddProperty(audioManager, Variant.From(in from5));
		StringName remoteCursorContainer = PropertyName.RemoteCursorContainer;
		NRemoteMouseCursorContainer from6 = RemoteCursorContainer;
		info.AddProperty(remoteCursorContainer, Variant.From(in from6));
		StringName inputManager = PropertyName.InputManager;
		NInputManager from7 = InputManager;
		info.AddProperty(inputManager, Variant.From(in from7));
		StringName hotkeyManager = PropertyName.HotkeyManager;
		NHotkeyManager from8 = HotkeyManager;
		info.AddProperty(hotkeyManager, Variant.From(in from8));
		StringName reactionWheel = PropertyName.ReactionWheel;
		NReactionWheel from9 = ReactionWheel;
		info.AddProperty(reactionWheel, Variant.From(in from9));
		StringName reactionContainer = PropertyName.ReactionContainer;
		NReactionContainer from10 = ReactionContainer;
		info.AddProperty(reactionContainer, Variant.From(in from10));
		StringName cursorManager = PropertyName.CursorManager;
		NCursorManager from11 = CursorManager;
		info.AddProperty(cursorManager, Variant.From(in from11));
		StringName debugAudio = PropertyName.DebugAudio;
		NDebugAudioManager from12 = DebugAudio;
		info.AddProperty(debugAudio, Variant.From(in from12));
		StringName debugSeedOverride = PropertyName.DebugSeedOverride;
		string from13 = DebugSeedOverride;
		info.AddProperty(debugSeedOverride, Variant.From(in from13));
		StringName startOnMainMenu = PropertyName.StartOnMainMenu;
		bool from14 = StartOnMainMenu;
		info.AddProperty(startOnMainMenu, Variant.From(in from14));
		StringName inspectRelicScreen = PropertyName.InspectRelicScreen;
		NInspectRelicScreen from15 = InspectRelicScreen;
		info.AddProperty(inspectRelicScreen, Variant.From(in from15));
		StringName inspectCardScreen = PropertyName.InspectCardScreen;
		NInspectCardScreen from16 = InspectCardScreen;
		info.AddProperty(inspectCardScreen, Variant.From(in from16));
		StringName feedbackScreen = PropertyName.FeedbackScreen;
		NSendFeedbackScreen from17 = FeedbackScreen;
		info.AddProperty(feedbackScreen, Variant.From(in from17));
		StringName worldEnvironment = PropertyName.WorldEnvironment;
		WorldEnvironment from18 = WorldEnvironment;
		info.AddProperty(worldEnvironment, Variant.From(in from18));
		StringName hitStop = PropertyName.HitStop;
		NHitStop from19 = HitStop;
		info.AddProperty(hitStop, Variant.From(in from19));
