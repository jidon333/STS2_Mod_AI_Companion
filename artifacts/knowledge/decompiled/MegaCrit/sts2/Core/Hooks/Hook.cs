...
public static class Hook
{
	...
	{
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.AfterCardChangedPiles(card, oldPile, source);
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.AfterCardChangedPilesLate(card, oldPile, source);
			model2.InvokeExecutionFinished();
		}
	}
	...
	{
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model2);
			await model2.AfterCardDrawnEarly(choiceContext, card, fromHandDraw);
			model2.InvokeExecutionFinished();
			choiceContext.PopModel(model2);
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model2);
			await model2.AfterCardDrawn(choiceContext, card, fromHandDraw);
			model2.InvokeExecutionFinished();
			choiceContext.PopModel(model2);
		}
	}
	...
	public static async Task AfterCardPlayed(CombatState combatState, PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		{
			choiceContext.PushModel(model2);
			await model2.AfterCardPlayed(choiceContext, cardPlay);
			model2.InvokeExecutionFinished();
			choiceContext.PopModel(model2);
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model2);
			await model2.AfterCardPlayedLate(choiceContext, cardPlay);
			model2.InvokeExecutionFinished();
			choiceContext.PopModel(model2);
		}
	}
	...
	public static async Task BeforeCombatStart(IRunState runState, CombatState? combatState)
	{
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.BeforeCombatStart();
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.BeforeCombatStartLate();
			model2.InvokeExecutionFinished();
		}
	}
	...
	public static async Task AfterCombatVictory(IRunState runState, CombatState? combatState, CombatRoom room)
	{
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.AfterCombatVictoryEarly(room);
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			await model2.AfterCombatVictory(room);
			model2.InvokeExecutionFinished();
		}
	}
	...
	public static async Task AfterDamageReceived(PlayerChoiceContext choiceContext, IRunState runState, CombatState? combatState, Creature target, DamageResult result, ValueProp props, Creature? dealer, CardModel? cardSource)
	{
		...
		{
			choiceContext.PushModel(model2);
			await model2.AfterDamageReceived(choiceContext, target, result, props, dealer, cardSource);
			choiceContext.PopModel(model2);
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in runState.IterateHookListeners(combatState))
		{
			choiceContext.PushModel(model2);
			await model2.AfterDamageReceivedLate(choiceContext, target, result, props, dealer, cardSource);
			choiceContext.PopModel(model2);
			model2.InvokeExecutionFinished();
		}
	}
	...
	public static async Task AfterEnergyReset(CombatState combatState, Player player)
	{
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			await model2.AfterEnergyReset(player);
			model2.InvokeExecutionFinished();
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			await model2.AfterEnergyResetLate(player);
			model2.InvokeExecutionFinished();
		}
	}
	...
	public static async Task BeforeFlush(CombatState combatState, Player player)
	{
		...
		{
			HookPlayerChoiceContext playerChoiceContext2 = new HookPlayerChoiceContext(item, netId.Value, player.Creature.CombatState, GameActionType.Combat);
			Task task = item.BeforeFlush(playerChoiceContext2, player);
			if (!(await playerChoiceContext2.AssignTaskAndWaitForPauseOrCompletion(task)))
			{
				tasksToAwait.Add(playerChoiceContext2.GameAction.CompletionTask);
			}
		}
		foreach (AbstractModel item2 in combatState.IterateHookListeners())
		{
			HookPlayerChoiceContext playerChoiceContext2 = new HookPlayerChoiceContext(item2, netId.Value, player.Creature.CombatState, GameActionType.Combat);
			Task task2 = item2.BeforeFlushLate(playerChoiceContext2, player);
			if (!(await playerChoiceContext2.AssignTaskAndWaitForPauseOrCompletion(task2)))
			{
				tasksToAwait.Add(playerChoiceContext2.GameAction.CompletionTask);
			}
		}
	...
	public static async Task BeforeHandDraw(CombatState combatState, Player player, PlayerChoiceContext playerChoiceContext)
	{
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			playerChoiceContext.PushModel(model2);
			await model2.BeforeHandDraw(player, playerChoiceContext, combatState);
			model2.InvokeExecutionFinished();
			playerChoiceContext.PopModel(model2);
		}
		foreach (AbstractModel model2 in combatState.IterateHookListeners())
		{
			playerChoiceContext.PushModel(model2);
			await model2.BeforeHandDrawLate(player, playerChoiceContext, combatState);
			model2.InvokeExecutionFinished();
			playerChoiceContext.PopModel(model2);
		}
	}
	...
	{
		foreach (AbstractModel model3 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model3);
			await model3.AfterPlayerTurnStartEarly(choiceContext, player);
			model3.InvokeExecutionFinished();
			choiceContext.PopModel(model3);
		}
		foreach (AbstractModel model3 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model3);
			await model3.AfterPlayerTurnStart(choiceContext, player);
			model3.InvokeExecutionFinished();
			choiceContext.PopModel(model3);
		}
		foreach (AbstractModel model3 in combatState.IterateHookListeners())
		{
			choiceContext.PushModel(model3);
			await model3.AfterPlayerTurnStartLate(choiceContext, player);
			model3.InvokeExecutionFinished();
			choiceContext.PopModel(model3);
		}
	}
	...
	public static async Task BeforeTurnEnd(CombatState combatState, CombatSide side)
	{
		...
		foreach (AbstractModel item in combatState.IterateHookListeners())
		{
			HookPlayerChoiceContext playerChoiceContext3 = new HookPlayerChoiceContext(item, netId.Value, combatState, GameActionType.Combat);
			Task task = item.BeforeTurnEndVeryEarly(playerChoiceContext3, side);
			if (!(await playerChoiceContext3.AssignTaskAndWaitForPauseOrCompletion(task)))
			{
				tasksToAwait.Add(playerChoiceContext3.GameAction.CompletionTask);
			}
		}
		foreach (AbstractModel item2 in combatState.IterateHookListeners())
		{
			HookPlayerChoiceContext playerChoiceContext3 = new HookPlayerChoiceContext(item2, netId.Value, combatState, GameActionType.Combat);
			Task task2 = item2.BeforeTurnEndEarly(playerChoiceContext3, side);
			if (!(await playerChoiceContext3.AssignTaskAndWaitForPauseOrCompletion(task2)))
			{
				tasksToAwait.Add(playerChoiceContext3.GameAction.CompletionTask);
			}
		}
		...
		{
			HookPlayerChoiceContext playerChoiceContext3 = new HookPlayerChoiceContext(item3, netId.Value, combatState, GameActionType.Combat);
			Task task3 = item3.BeforeTurnEnd(playerChoiceContext3, side);
			if (!(await playerChoiceContext3.AssignTaskAndWaitForPauseOrCompletion(task3)))
			{
				tasksToAwait.Add(playerChoiceContext3.GameAction.CompletionTask);
			}
		}
	...
	public static async Task AfterTurnEnd(CombatState combatState, CombatSide side)
	{
		...
		foreach (AbstractModel item in combatState.IterateHookListeners())
		{
			HookPlayerChoiceContext playerChoiceContext2 = new HookPlayerChoiceContext(item, netId.Value, combatState, GameActionType.Combat);
			Task task = item.AfterTurnEnd(playerChoiceContext2, side);
			if (!(await playerChoiceContext2.AssignTaskAndWaitForPauseOrCompletion(task)))
			{
				tasksToAwait.Add(playerChoiceContext2.GameAction.CompletionTask);
			}
		...
		foreach (AbstractModel item2 in combatState.IterateHookListeners())
		{
			HookPlayerChoiceContext playerChoiceContext2 = new HookPlayerChoiceContext(item2, netId.Value, combatState, GameActionType.Combat);
			Task task2 = item2.AfterTurnEndLate(playerChoiceContext2, side);
			if (!(await playerChoiceContext2.AssignTaskAndWaitForPauseOrCompletion(task2)))
			{
				tasksToAwait.Add(playerChoiceContext2.GameAction.CompletionTask);
			}
