...
public class NetFullCombatState : IPacketSerializable
{
	public struct CreatureState : IPacketSerializable
	{
		...
		public CreatureState Anonymized()
		{
			...
			result.playerId = (playerId.HasValue ? new ulong?(IdAnonymizer.Anonymize(playerId.Value)) : null);
			return result;
		}
	}
	...
	public struct OrbState : IPacketSerializable
	{
		...
		{
			OrbState result = default(OrbState);
			result.id = orb.Id;
			result.passive = (int)orb.PassiveVal;
			result.evoke = (int)orb.EvokeVal;
			return result;
		}
	}
	...
	public struct CombatPileState : IPacketSerializable
	{
		...
		{
			CombatPileState result = default(CombatPileState);
			result.pileType = pile.Type;
			result.cards = new List<CardState>();
	...
	public struct CardState : IPacketSerializable
	{
		...
		{
			CardState result = default(CardState);
			result.card = card.ToSerializable();
			result.affliction = card.Affliction?.Id;
			result.afflictionCount = card.Affliction?.Amount ?? 0;
			result.keywords = ((card.Keywords.Count > 0) ? card.Keywords.ToList() : null);
			return result;
		}
	}
	...
	public static NetFullCombatState FromRun(IRunState runState, GameAction? justFinishedAction)
	{
		...
		netFullCombatState.lastExecutedHookId = ((justFinishedAction is GenericHookGameAction genericHookGameAction) ? new uint?(genericHookGameAction.HookId) : null);
		...
		{
			CreatureState creatureState = default(CreatureState);
			creatureState.monsterId = item3.Monster?.Id;
			creatureState.playerId = item3.Player?.NetId;
			creatureState.currentHp = item3.CurrentHp;
			creatureState.maxHp = item3.MaxHp;
			creatureState.block = item3.Block;
			creatureState.powers = new List<PowerState>();
			CreatureState item = creatureState;
		...
		foreach (Player player in runState.Players)
		{
			...
			playerState.playerId = player.NetId;
			playerState.characterId = player.Character.Id;
			playerState.energy = player.PlayerCombatState?.Energy ?? 0;
			playerState.stars = player.PlayerCombatState?.Stars ?? 0;
			playerState.maxPotionCount = player.MaxPotionCount;
			playerState.gold = player.Gold;
			playerState.piles = new List<CombatPileState>();
			playerState.potions = new List<PotionState>();
			playerState.relics = new List<RelicState>();
			playerState.orbs = new List<OrbState>();
			playerState.rngSet = player.PlayerRng.ToSerializable();
			playerState.oddsSet = player.PlayerOdds.ToSerializable();
			playerState.relicGrabBag = player.RelicGrabBag.ToSerializable();
			PlayerState item2 = playerState;
