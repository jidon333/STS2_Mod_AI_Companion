...
public class ProgressState
{
	...
	{
		string epochId2 = epochId;
		SerializableEpoch serializableEpoch = _epochs.FirstOrDefault((SerializableEpoch e) => e.Id == epochId2);
		if (serializableEpoch != null)
		{
			serializableEpoch.SetObtained(EpochState.Obtained);
			return;
		}
		_epochs.Add(new SerializableEpoch(epochId2, EpochState.ObtainedNoSlot));
		FilterAndSortEpochs();
	}
	...
	public void ObtainEpochOverride(string epochId, EpochState state)
	{
		...
		{
			_epochs.Add(new SerializableEpoch(epochId2, state));
			FilterAndSortEpochs();
		}
	}
	...
	public void UnlockSlot(string epochId)
	{
		...
		if (serializableEpoch == null)
		{
			_epochs.Add(new SerializableEpoch(epochId2, EpochState.NotObtained));
			FilterAndSortEpochs();
			return;
		}
		if (serializableEpoch.State == EpochState.ObtainedNoSlot)
		{
			Log.Info("Attempted to get slot " + epochId2 + " but we already have an Epoch! Set to Obtained");
			serializableEpoch.State = EpochState.Obtained;
			return;
		}
		Log.Error($"Slot unlocked for {epochId2} but it's in an invalid state: {serializableEpoch.State}");
	}
	...
	public void RevealEpoch(string epochId)
	{
		...
		if (serializableEpoch == null)
		{
			throw new InvalidOperationException($"Invalid epoch {epochId2} passed to {"RevealEpoch"}!");
		}
	...
	{
		string epochId2 = epochId;
		return _epochs.Any((SerializableEpoch e) => e.Id == epochId2);
	}
