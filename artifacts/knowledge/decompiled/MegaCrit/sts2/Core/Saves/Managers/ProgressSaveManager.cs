...
public class ProgressSaveManager
{
	...
	private void IncrementEncounterLoss(ModelId characterId, ModelId encounterId)
	{
		...
		FightStats fightStats = orCreateEncounterStats.FightStats.FirstOrDefault((FightStats f) => f.Character == characterId2);
		if (fightStats == null)
		{
			Log.Info($"{characterId2} fought {encounterId} for the first time and LOST :(");
		...
		{
			orCreateEncounterStats.IncrementLoss(characterId2);
		}
	}
	...
	private void IncrementEnemyFightLoss(ModelId characterId, ModelId monster)
	{
		...
		FightStats fightStats = orCreateEnemyStats.FightStats.FirstOrDefault((FightStats f) => f.Character == characterId2);
		if (fightStats == null)
		{
			Log.Info($"{characterId2} fought {monster} for the first time and LOST >:(");
			FightStats item = new FightStats
			{
				Character = characterId2,
				Wins = 0,
		...
		{
			orCreateEnemyStats.IncrementLoss(characterId2);
		}
	}
	...
	public void UpdateWithRunData(SerializableRun serializableRun, bool victory)
	{
		...
		for (int i = 0; i < serializableRun.MapPointHistory.Count; i++)
		{
			if (i >= serializableRun.Acts.Count)
			{
			...
			{
				ModelId id = serializableRun.Acts[i].Id;
		...
		if (victory)
		{
			int num = ScoreUtility.CalculateScore(serializableRun, victory);
			Progress.ArchitectDamage += num;
			if (flag)
			{
	...
	{
		EpochModel epoch2 = epoch;
		if (Progress.IsEpochObtained(epoch2.Id))
		{
			Log.Info("Player already has Epoch: " + epoch2.Id);
			return false;
		}
		Progress.ObtainEpoch(epoch2.Id);
		NGame.Instance?.AddChildSafely(NGainEpochVfx.Create(epoch2));
		if (GetRevealableEpochs().All((SerializableEpoch e) => e.Id != epoch2.Id))
		{
			string text = "Epoch " + epoch2.Id + " was obtained, but is not yet revealable by the player!";
			Log.Warn(text);
