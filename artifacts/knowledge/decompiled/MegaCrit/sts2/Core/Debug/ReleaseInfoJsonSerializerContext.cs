...
internal class ReleaseInfoJsonSerializerContext : JsonSerializerContext, IJsonTypeInfoResolver
{
	...
	private JsonTypeInfo<ReleaseInfo> Create_ReleaseInfo(JsonSerializerOptions options)
	{
		...
		if (!TryGetTypeInfoForRuntimeCustomConverter(options2, out JsonTypeInfo<ReleaseInfo> jsonTypeInfo))
		{
			...
			jsonObjectInfoValues.PropertyMetadataInitializer = (JsonSerializerContext _) => ReleaseInfoPropInit(options2);
