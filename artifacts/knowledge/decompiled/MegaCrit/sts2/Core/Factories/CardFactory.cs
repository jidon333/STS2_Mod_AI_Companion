...
public static class CardFactory
{
	...
	public static IEnumerable<CardModel> GetDistinctForCombat(Player player, IEnumerable<CardModel> cards, int count, Rng rng)
	{
		...
		cards = FilterForPlayerCount(player2.RunState, cards);
	...
	{
		CardModel original2 = original;
		IEnumerable<CardModel> source = originalOptions;
		CardRarity rarity = original2.Rarity;
		if ((uint)(rarity - 8) > 1u)
		{
		...
		source = source.Where((CardModel c) => c.Id != original2.Id).ToList();
		CardModel[] array = FilterForPlayerCount(original2.Owner.RunState, source).ToArray();
		if (array.Length == 0)
		{
	...
	private static CardRarity RollForRarity(Player player, CardRarityOddsType rollMethod, CardCreationSource source, HashSet<CardRarity> allowedRarities, bool forceRarityOddsChange)
	{
		...
		while (!allowedRarities.Contains(cardRarity) && cardRarity != 0)
		{
