...
public static class ReflectionHelper
{
	...
	{
		Type parentType2 = parentType;
		return from type in AllTypes.Concat(ModTypes)
			where (object)type != null && !type.IsAbstract && !type.IsInterface && InheritsOrImplements(type, parentType2)
			select type;
	}
	...
	{
		Type parentType2 = parentType;
		return ModTypes.Where((Type type) => (object)type != null && !type.IsAbstract && !type.IsInterface && InheritsOrImplements(type, parentType2));
	}
