...
public class NDebugAudioManager : Node
{
	...
	public int Play(string streamName, float volume = 1f, PitchVariance variance = PitchVariance.None)
	{
		...
		playingSound.callable = callable;
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			int from = Play(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<float>(in args[1]), VariantUtils.ConvertTo<PitchVariance>(in args[2]));
		...
		{
			float from2 = GetRandomPitchScale(VariantUtils.ConvertTo<PitchVariance>(in args[0]));
