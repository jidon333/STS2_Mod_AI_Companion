...
public class AtlasResourceLoader : ResourceFormatLoader
{
	...
	protected override bool InvokeGodotClassMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		...
		{
			string[] from = _GetRecognizedExtensions();
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName._HandlesType && args.Count == 1)
		{
			bool from2 = _HandlesType(VariantUtils.ConvertTo<StringName>(in args[0]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName._GetResourceType && args.Count == 1)
		{
			string from3 = _GetResourceType(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (method == MethodName._RecognizePath && args.Count == 2)
		{
			bool from4 = _RecognizePath(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<StringName>(in args[1]));
			ret = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (method == MethodName._Exists && args.Count == 1)
		{
			bool from5 = _Exists(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from5);
			return true;
		}
		if (method == MethodName._Load && args.Count == 4)
		{
			Variant from6 = _Load(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]), VariantUtils.ConvertTo<bool>(in args[2]), VariantUtils.ConvertTo<int>(in args[3]));
			ret = VariantUtils.CreateFrom(in from6);
			return true;
		}
		if (method == MethodName._GetDependencies && args.Count == 2)
		{
			string[] from7 = _GetDependencies(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<bool>(in args[1]));
			ret = VariantUtils.CreateFrom(in from7);
			return true;
		}
		if (method == MethodName.IsSpritePath && args.Count == 1)
		{
			bool from8 = IsSpritePath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from8);
			return true;
		}
		if (method == MethodName.HasFallback && args.Count == 2)
		{
			bool from9 = HasFallback(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from9);
			return true;
		}
		if (method == MethodName.LoadFallback && args.Count == 2)
		{
			Texture2D from10 = LoadFallback(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from10);
			return true;
		}
		if (method == MethodName.GetFallbackPath && args.Count == 2)
		{
			string from11 = GetFallbackPath(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from11);
			return true;
		}
		if (method == MethodName.GetRelicFallbackPath && args.Count == 1)
		{
			string from12 = GetRelicFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from12);
			return true;
		}
		if (method == MethodName.GetPowerFallbackPath && args.Count == 1)
		{
			string from13 = GetPowerFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from13);
			return true;
		}
		if (method == MethodName.GetCardFallbackPath && args.Count == 1)
		{
			string from14 = GetCardFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from14);
			return true;
		}
		if (method == MethodName.GetPotionFallbackPath && args.Count == 1)
		{
			string from15 = GetPotionFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from15);
			return true;
		}
		if (method == MethodName.GetMissingTexture && args.Count == 1)
		{
			Variant from16 = GetMissingTexture(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from16);
			return true;
		}
	...
	internal static bool InvokeGodotClassStaticMethod(in godot_string_name method, NativeVariantPtrArgs args, out godot_variant ret)
	{
		if (method == MethodName.IsSpritePath && args.Count == 1)
		{
			bool from = IsSpritePath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from);
			return true;
		}
		if (method == MethodName.HasFallback && args.Count == 2)
		{
			bool from2 = HasFallback(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from2);
			return true;
		}
		if (method == MethodName.LoadFallback && args.Count == 2)
		{
			Texture2D from3 = LoadFallback(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from3);
			return true;
		}
		if (method == MethodName.GetFallbackPath && args.Count == 2)
		{
			string from4 = GetFallbackPath(VariantUtils.ConvertTo<string>(in args[0]), VariantUtils.ConvertTo<string>(in args[1]));
			ret = VariantUtils.CreateFrom(in from4);
			return true;
		}
		if (method == MethodName.GetRelicFallbackPath && args.Count == 1)
		{
			string from5 = GetRelicFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from5);
			return true;
		}
		if (method == MethodName.GetPowerFallbackPath && args.Count == 1)
		{
			string from6 = GetPowerFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from6);
			return true;
		}
		if (method == MethodName.GetCardFallbackPath && args.Count == 1)
		{
			string from7 = GetCardFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from7);
			return true;
		}
		if (method == MethodName.GetPotionFallbackPath && args.Count == 1)
		{
			string from8 = GetPotionFallbackPath(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from8);
			return true;
		}
		if (method == MethodName.GetMissingTexture && args.Count == 1)
		{
			Variant from9 = GetMissingTexture(VariantUtils.ConvertTo<string>(in args[0]));
			ret = VariantUtils.CreateFrom(in from9);
			return true;
		}
