...
public class ConnectionStats
{
	...
	public HeartbeatRequestMessage GenerateHeartbeat(ulong timeMsec)
	{
		...
		HeartbeatStatus heartbeatStatus = default(HeartbeatStatus);
		heartbeatStatus.counter = _nextIndex;
		heartbeatStatus.sentMsec = timeMsec;
		HeartbeatStatus value = heartbeatStatus;
		HeartbeatStatus? heartbeatStatus2 = _statuses[num];
		if (heartbeatStatus2.HasValue && !heartbeatStatus2.Value.receivedMsec.HasValue)
		{
			_logger.VeryDebug($"Heartbeat {heartbeatStatus2.Value.counter} ({heartbeatStatus2.Value.sentMsec}) was never received, marking as lost");
			OnPacketLost();
		}
		_statuses[num] = value;
		HeartbeatRequestMessage heartbeatRequestMessage = default(HeartbeatRequestMessage);
		heartbeatRequestMessage.counter = _nextIndex;
		HeartbeatRequestMessage result = heartbeatRequestMessage;
		_nextIndex++;
		return result;
	}
