...
public class RunLocationTargetedMessageBuffer
{
	...
	{
		MessageHandlerDelegate<T> handler2 = handler;
		_logger.VeryDebug($"Register message handler {handler2} for {typeof(T)}");
	...
	private void HandleMessage<T>(T message, ulong senderId) where T : INetMessage, IRunLocationTargetedMessage
	{
		...
		BlockedMessage blockedMessage = default(BlockedMessage);
		blockedMessage.location = message.Location;
		blockedMessage.message = message;
		blockedMessage.messageType = message.GetType();
		blockedMessage.senderId = senderId;
