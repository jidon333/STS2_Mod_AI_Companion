...
public class StartRunLobby
{
	...
	private void HandleClientLobbyJoinRequestMessage(ClientLobbyJoinRequestMessage message, ulong senderId)
	{
		...
		try
		{
			...
			message3.lobbyPlayer = lobbyPlayer.Value;
	...
	private LobbyPlayer? TryAddPlayerInFirstAvailableSlot(SerializableUnlockState unlockState, int maxAscensionUnlocked, ulong playerId)
	{
		...
		lobbyPlayer.character = ModelDb.Character<Ironclad>();
		lobbyPlayer.id = playerId;
		lobbyPlayer.slotId = num;
		lobbyPlayer.maxMultiplayerAscensionUnlocked = maxAscensionUnlocked;
		lobbyPlayer.unlockState = unlockState;
		LobbyPlayer lobbyPlayer2 = lobbyPlayer;
		Players.Add(lobbyPlayer2);
		return lobbyPlayer2;
	}
	...
	private void OnConnectedToClientAsHost(ulong playerId)
	{
		...
		else
		{
			...
			NetService.SendMessage(message, playerId);
			TaskHelper.RunSafely(BeginHandshakeTimeout(connectingPlayer2));
		}
	}
