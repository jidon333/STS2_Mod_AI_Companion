...
public class RewardSynchronizer : IDisposable
{
	...
	private void SyncLocalCardEvent(CardModel card, bool skipped)
	{
		...
		RewardObtainedMessage rewardObtainedMessage = default(RewardObtainedMessage);
		rewardObtainedMessage.rewardType = RewardType.Card;
		rewardObtainedMessage.cardModel = card;
		rewardObtainedMessage.wasSkipped = skipped;
		rewardObtainedMessage.location = _messageBuffer.CurrentLocation;
		RewardObtainedMessage message = rewardObtainedMessage;
	...
	private void SyncLocalRelicEvent(RelicModel relic, bool skipped)
	{
		...
		RewardObtainedMessage rewardObtainedMessage = default(RewardObtainedMessage);
		rewardObtainedMessage.rewardType = RewardType.Relic;
		rewardObtainedMessage.relicModel = relic;
		rewardObtainedMessage.wasSkipped = skipped;
		rewardObtainedMessage.location = _messageBuffer.CurrentLocation;
		RewardObtainedMessage message = rewardObtainedMessage;
		_gameService.SendMessage(message);
	...
	private void SyncLocalPotionEvent(PotionModel potion, bool skipped)
	{
		...
		RewardObtainedMessage rewardObtainedMessage = default(RewardObtainedMessage);
		rewardObtainedMessage.rewardType = RewardType.Potion;
		rewardObtainedMessage.potionModel = potion;
		rewardObtainedMessage.wasSkipped = skipped;
		rewardObtainedMessage.location = _messageBuffer.CurrentLocation;
		RewardObtainedMessage message = rewardObtainedMessage;
		_gameService.SendMessage(message);
	...
	public void SyncLocalObtainedGold(int goldAmount)
	{
		...
		RewardObtainedMessage rewardObtainedMessage = default(RewardObtainedMessage);
		rewardObtainedMessage.rewardType = RewardType.Gold;
		rewardObtainedMessage.goldAmount = goldAmount;
		rewardObtainedMessage.wasSkipped = false;
		rewardObtainedMessage.location = _messageBuffer.CurrentLocation;
		RewardObtainedMessage message = rewardObtainedMessage;
		_gameService.SendMessage(message);
	}
	...
	public void SyncLocalGoldLost(int goldLost)
	{
		...
		GoldLostMessage goldLostMessage = default(GoldLostMessage);
		goldLostMessage.goldLost = goldLost;
		goldLostMessage.location = _messageBuffer.CurrentLocation;
		GoldLostMessage message = goldLostMessage;
		_gameService.SendMessage(message);
	}
	...
	private void HandleRewardObtainedMessage(RewardObtainedMessage message, ulong senderId)
	{
		...
		{
			BufferedMessage bufferedMessage = default(BufferedMessage);
			bufferedMessage.senderId = senderId;
			bufferedMessage.rewardMessage = message;
			BufferedMessage item = bufferedMessage;
	...
	private void HandleGoldLostMessage(GoldLostMessage message, ulong senderId)
	{
		if (CombatManager.Instance.IsInProgress)
		{
			BufferedMessage bufferedMessage = default(BufferedMessage);
			bufferedMessage.senderId = senderId;
			bufferedMessage.goldLostMessage = message;
			BufferedMessage item = bufferedMessage;
			_bufferedMessages.Add(item);
		}
		else
	...
	private void HandleCardRemovedMessage(CardRemovedMessage message, ulong senderId)
	{
		if (CombatManager.Instance.IsInProgress)
		{
			BufferedMessage bufferedMessage = default(BufferedMessage);
			bufferedMessage.senderId = senderId;
			bufferedMessage.cardRemovedMessage = message;
			BufferedMessage item = bufferedMessage;
			_bufferedMessages.Add(item);
		}
		else
	...
	private void HandlePaelsWingSacrifice(PaelsWingSacrificeMessage message, ulong senderId)
	{
		if (CombatManager.Instance.IsInProgress)
		{
			BufferedMessage bufferedMessage = default(BufferedMessage);
			bufferedMessage.senderId = senderId;
			bufferedMessage.paelsWingSacrificeMessage = message;
			BufferedMessage item = bufferedMessage;
			_bufferedMessages.Add(item);
			return;
		}
