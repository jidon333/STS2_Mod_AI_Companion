...
public struct InitialGameInfoMessage : INetMessage, IPacketSerializable
{
	...
	{
		InitialGameInfoMessage result = default(InitialGameInfoMessage);
		result.version = ReleaseInfoManager.Instance.ReleaseInfo?.Version ?? GitHelper.ShortCommitId ?? "UNKNOWN";
		result.idDatabaseHash = ModelIdSerializationCache.Hash;
		result.mods = ModManager.GetModNameList();
		return result;
	}
