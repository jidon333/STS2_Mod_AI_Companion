...
public static class CardSelectCmd
{
	...
	{
		List<CardCreationResult> cards2 = cards;
		if (CombatManager.Instance.IsEnding)
		{
			return Array.Empty<CardModel>();
		}
		List<CardModel> result;
		if (!prefs.RequireManualConfirmation && cards2.Count <= prefs.MinSelect)
		{
			result = cards2.Select((CardCreationResult c) => c.Card).ToList();
		}
		else
		{
			uint choiceId = RunManager.Instance.PlayerChoiceSynchronizer.ReserveChoiceId(player);
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					IEnumerable<CardModel> options = cards2.Select((CardCreationResult c) => c.Card);
					result = (await Selector.GetSelectedCards(options, prefs.MinSelect, prefs.MaxSelect)).ToList();
				}
				else
				{
					NSimpleCardSelectScreen nSimpleCardSelectScreen = NSimpleCardSelectScreen.Create(cards2, prefs);
					NOverlayStack.Instance.Push(nSimpleCardSelectScreen);
					result = (await nSimpleCardSelectScreen.CardsSelected()).ToList();
				}
				List<int> indexes = result.Select((CardModel c) => cards2.FindIndex((CardCreationResult r) => r.Card == c)).ToList();
	...
	{
		IReadOnlyList<CardModel> cards2 = cards;
		if (CombatManager.Instance.IsEnding)
		{
			return Array.Empty<CardModel>();
		}
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					result = (await Selector.GetSelectedCards(cards2, prefs.MinSelect, prefs.MaxSelect)).ToList();
				}
				else
				{
					NPlayerHand.Instance?.CancelAllCardPlay();
				...
				List<int> indexes = result.Select(cards2.IndexOf<CardModel>).ToList();
				RunManager.Instance.PlayerChoiceSynchronizer.SyncLocalChoice(player, choiceId, PlayerChoiceResult.FromIndexes(indexes));
			}
			else
			{
				result = (from i in (await RunManager.Instance.PlayerChoiceSynchronizer.WaitForRemoteChoice(player, choiceId)).AsIndexes()
					select cards2[i]).ToList();
			}
			await context.SignalPlayerChoiceEnded();
		}
		LogChoice(player, result);
	...
	{
		EnchantmentModel enchantment2 = enchantment;
		Func<CardModel?, bool> additionalFilter2 = additionalFilter;
		IReadOnlyList<CardModel> cards = PileType.Deck.GetPile(player).Cards.Where((CardModel c) => enchantment2.CanEnchant(c) && (additionalFilter2?.Invoke(c) ?? true)).ToList();
		return await FromDeckForEnchantment(cards, enchantment2, amount, prefs);
	}
	...
	{
		EnchantmentModel enchantment2 = enchantment;
		if (cards.Any((CardModel c) => c.Pile.Type != PileType.Deck || !enchantment2.CanEnchant(c)))
		{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				...
				{
					NDeckEnchantSelectScreen nDeckEnchantSelectScreen = NDeckEnchantSelectScreen.ShowScreen(list, enchantment2, amount, prefs);
	...
	{
		Func<CardModel, bool> filter2 = filter;
		List<CardModel> deck = PileType.Deck.GetPile(player).Cards.ToList();
		return FromDeckGeneric(player, prefs, (CardModel c) => c.IsRemovable && (filter2 == null || filter2(c)), (CardModel c) => (c.Type != CardType.Curse) ? deck.IndexOf(c) : (-999999999));
	}
