...
public static class CreatureCmd
{
	...
	public static async Task<IEnumerable<DamageResult>> Damage(PlayerChoiceContext choiceContext, IEnumerable<Creature> targets, decimal amount, ValueProp props, Creature? dealer, CardModel? cardSource)
	{
		...
		foreach (DamageResult unblockedDamageResult2 in results)
		{
			...
			if (unblockedDamageResult2.WasFullyBlocked)
			{
				if (!CombatManager.Instance.IsInProgress)
				{
				...
				node?.AddChildSafely(NBlockSparkVfx.Create(unblockedDamageResult2.Receiver));
				node?.AddChildSafely(NDamageBlockedVfx.Create(unblockedDamageResult2.Receiver));
				NGame.Instance?.ScreenShake(ShakeStrength.Weak, ShakeDuration.Short);
			}
		}
	...
	public static async Task Heal(Creature creature, decimal amount, bool playAnim = true)
	{
		...
		amount = Hook.ModifyHealAmount(creature2.Player?.RunState ?? creature2.CombatState?.RunState ?? NullRunState.Instance, creature2.CombatState, creature2, amount);
		...
		if (playAnim)
		{
			if (CombatManager.Instance.IsInProgress || creature2.Player?.RunState.CurrentRoom is CombatRoom)
			{
				...
				(NCombatRoom.Instance?.CombatVfxContainer)?.AddChildSafely(NHealNumVfx.Create(creature2, amount));
				if (isDead)
				{
			...
			else if (LocalContext.IsMe(creature2))
			{
				...
				{
					PlayerFullscreenHealVfx.Play(creature2.Player, amount, NMerchantRoom.Instance);
				}
			}
