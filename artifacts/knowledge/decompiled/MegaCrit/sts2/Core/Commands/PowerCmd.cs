...
public static class PowerCmd
{
	...
	public static async Task Apply(PowerModel power, Creature target, decimal amount, Creature? applier, CardModel? cardSource, bool silent = false)
	{
		...
		decimal modifiedAmount2 = amount;
		IEnumerable<AbstractModel> givenModifiers = null;
		if (applier != null && combatState.ContainsCreature(applier))
		{
			modifiedAmount2 = Hook.ModifyPowerAmountGiven(combatState, power, applier, modifiedAmount2, target, cardSource, out givenModifiers);
		}
		modifiedAmount2 = Hook.ModifyPowerAmountReceived(combatState, power, target, modifiedAmount2, applier, out IEnumerable<AbstractModel> receivedModifiers);
		await power.BeforeApplied(target, modifiedAmount2, applier, cardSource);
		if (modifiedAmount2 != 0m)
		{
			CombatManager.Instance.History.PowerReceived(combatState, power, modifiedAmount2, applier);
		}
		power.ApplyInternal(target, modifiedAmount2, silent);
		...
		if (modifiedAmount2 != 0m)
		{
			...
			await Hook.AfterPowerAmountChanged(combatState, power, modifiedAmount2, applier, cardSource);
		}
	}
	...
	public static async Task<int> ModifyAmount(PowerModel power, decimal offset, Creature? applier, CardModel? cardSource, bool silent = false)
	{
		...
		decimal modifiedOffset2 = offset;
		IEnumerable<AbstractModel> modifiers = null;
		if (applier != null && combatState.ContainsCreature(applier))
		{
			modifiedOffset2 = Hook.ModifyPowerAmountGiven(combatState, power, applier, modifiedOffset2, owner, cardSource, out modifiers);
		}
		modifiedOffset2 = Hook.ModifyPowerAmountReceived(combatState, power, owner, modifiedOffset2, applier, out IEnumerable<AbstractModel> receivedModifiers);
		CombatManager.Instance.History.PowerReceived(combatState, power, modifiedOffset2, applier);
		int newAmount = power.Amount + (int)modifiedOffset2;
		...
		await Hook.AfterModifyingPowerAmountReceived(combatState, receivedModifiers, power);
		if ((int)modifiedOffset2 != 0)
		{
			await Hook.AfterPowerAmountChanged(combatState, power, modifiedOffset2, applier, cardSource);
		}
