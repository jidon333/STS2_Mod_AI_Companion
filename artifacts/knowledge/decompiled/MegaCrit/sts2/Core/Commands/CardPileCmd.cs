	...
	public static async Task<IReadOnlyList<CardPileAddResult>> Add(IEnumerable<CardModel> cards, CardPile newPile, CardPilePosition position = CardPilePosition.Bottom, AbstractModel? source = null, bool skipVisuals = false)
	{
		...
		if (cardNodes.Count != 0)
		{
			...
			foreach (NCard cardNode in cardNodes)
			{
				...
				case PileType.Exhaust:
					...
					{
						AppendPileLerpTween(tween, cardNode, PileType.Play, oldPile3);
					...
					tween.Chain().TweenCallback(Callable.From(delegate
					{
						NCombatRoom.Instance.Ui.AddChildSafely(NExhaustVfx.Create(cardNode));
					}));
					tween.Parallel().TweenProperty(cardNode, "modulate", StsColors.exhaustGray, (SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.2f : 0.3f);
					tween.Chain().TweenCallback(Callable.From(cardNode.QueueFreeSafely));
					break;
				case PileType.Hand:
					AppendPileLerpTween(tween, cardNode, card2.Pile.Type, oldPile3);
					...
					break;
				case PileType.Play:
					AppendPlayPileLerpTween(tween, cardNode, oldPile3);
					break;
				default:
					tween.TweenCallback(Callable.From(delegate
					{
						Node node = ((card2.Pile.Type != PileType.Deck) ? NCombatRoom.Instance.CombatVfxContainer : NRun.Instance.GlobalUi.TopBar.TrailContainer);
		...
		if (cardsWithoutNodesChangingPiles.Count != 0)
		{
			foreach (CardModel card in cardsWithoutNodesChangingPiles)
			{
				...
				Node vfxContainer = ((card.Pile.Type != PileType.Deck) ? NCombatRoom.Instance.CombatVfxContainer : NRun.Instance.GlobalUi.TopBar.TrailContainer);
				if (tween != null)
				{
					tween.TweenCallback(Callable.From(delegate
					{
