...
public abstract class ActModel : AbstractModel
{
	...
	public void GenerateRooms(Rng rng, UnlockState unlockState, bool isMultiplayer = false)
	{
		...
		{
			list.RemoveAll(delegate(EventModel e)
			{
				EventModel e4 = e;
				return Event1Epoch.Events.Any((EventModel ev) => ev.Id == e4.Id);
			});
		}
		if (!unlockState.IsEpochRevealed<Event2Epoch>())
		{
			...
			{
				EventModel e3 = e;
				return Event2Epoch.Events.Any((EventModel ev) => ev.Id == e3.Id);
			});
		}
		if (!unlockState.IsEpochRevealed<Event3Epoch>())
		{
			...
			{
				EventModel e2 = e;
				return Event3Epoch.Events.Any((EventModel ev) => ev.Id == e2.Id);
			});
		}
	...
	{
		ICollection<EncounterModel> encounters2 = encounters;
		EncounterModel encounterModel = grabBag.GrabAndRemove(rng, (EncounterModel e) => !e.SharesTagsWith(encounters2.LastOrDefault()) && e != encounters2.LastOrDefault());
		...
		if (encounterModel != null)
		{
			encounters2.Add(encounterModel);
		}
	}
