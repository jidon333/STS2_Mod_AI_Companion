...
public sealed class Conflagration : CardModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(new DynamicVar[3]
	{
		...
		new CalculatedDamageVar(ValueProp.Move).WithMultiplier(delegate(CardModel card, Creature? _)
		{
			...
			return CombatManager.Instance.History.CardPlaysFinished.Count(delegate(CardPlayFinishedEntry e)
			{
				if (!e.HappenedThisTurn(card2.CombatState))
				{
					return false;
				}
				...
				return (e.CardPlay.Card.Owner == card2.Owner) ? true : false;
			});
		})
	});
