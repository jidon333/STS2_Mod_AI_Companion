...
public sealed class Finisher : CardModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new global::_003C_003Ez__ReadOnlyArray<DynamicVar>(new DynamicVar[4]
	{
		...
		new CalculatedVar("CalculatedHits").WithMultiplier(delegate(CardModel card, Creature? _)
		{
			...
			return CombatManager.Instance.History.CardPlaysFinished.Count((CardPlayFinishedEntry e) => e.HappenedThisTurn(card2.CombatState) && e.CardPlay.Card.Type == CardType.Attack && e.CardPlay.Card.Owner == card2.Owner);
		})
	});
