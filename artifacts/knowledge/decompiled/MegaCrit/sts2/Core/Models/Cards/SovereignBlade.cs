...
public sealed class SovereignBlade : CardModel
{
	...
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		AttackCommand attack2 = DamageCmd.Attack(base.DynamicVars.Damage.BaseValue).FromCard(this).WithHitCount(base.DynamicVars.Repeat.IntValue)
		...
		if (HasSeekingEdge)
		{
			attack2 = attack2.TargetingAllOpponents(base.CombatState).BeforeDamage(delegate
			{
				NSovereignBladeVfx vfxNode2 = GetVfxNode(base.Owner, this);
				IReadOnlyList<Creature> hittableEnemies = base.CombatState.HittableEnemies;
				if (hittableEnemies.Count > 0)
				{
					NCreature nCreature2 = NCombatRoom.Instance?.GetCreatureNode(hittableEnemies[0]);
		...
		else
		{
			...
			attack2 = attack2.Targeting(cardPlay2.Target).BeforeDamage(delegate
			{
				NSovereignBladeVfx vfxNode = GetVfxNode(base.Owner, this);
				NCreature nCreature = NCombatRoom.Instance?.GetCreatureNode(cardPlay2.Target);
		...
		await attack2.Execute(choiceContext);
		ParryPower power = base.Owner.Creature.GetPower<ParryPower>();
		if (power != null)
		{
			await power.AfterSovereignBladePlayed(base.Owner.Creature, attack2.Results);
		}
	}
