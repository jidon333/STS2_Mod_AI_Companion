...
public sealed class LastingCandy : RelicModel
{
	...
	{
		List<CardCreationResult> options2 = options;
		...
		IEnumerable<CardModel> customCardPool = from c in creationOptions.GetPossibleCards(player)
			where c.Type == CardType.Power && options2.TrueForAll((CardCreationResult o) => o.originalCard.Id != c.Id)
			select c;
		CardCreationOptions options3 = new CardCreationOptions(customCardPool, CardCreationSource.Other, creationOptions.RarityOdds).WithFlags(CardCreationFlags.NoModifyHooks | CardCreationFlags.NoCardPoolModifications);
		CardModel cardModel = CardFactory.CreateForReward(base.Owner, 1, options3).FirstOrDefault()?.Card;
		if (cardModel != null)
		{
			CardCreationResult cardCreationResult = new CardCreationResult(cardModel);
			cardCreationResult.ModifyCard(cardModel, this);
			options2.Add(cardCreationResult);
		}
