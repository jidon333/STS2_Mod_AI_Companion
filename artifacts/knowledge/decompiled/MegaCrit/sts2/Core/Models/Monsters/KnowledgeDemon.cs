...
public sealed class KnowledgeDemon : MonsterModel
{
	...
	private async Task ChooseCurse(Creature target)
	{
		...
		{
			CardModel cardModel = base.CombatState.CreateCard((CardModel)c, target2.Player);
		...
		await ((IChoosable)(await CardSelectCmd.FromChooseACardScreen(new BlockingPlayerChoiceContext(), cards, target2.Player))).OnChosen();
	}
