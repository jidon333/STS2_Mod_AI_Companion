...
public sealed class TheFutureOfPotions : EventModel
{
	...
	private async Task Trade(PotionModel potion)
	{
		...
		CardRarity targetRarity = GetCardRarity(potion2);
		await PotionCmd.Discard(potion2);
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new global::_003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(base.Owner.Character.CardPool), (CardModel c) => c.Rarity == targetRarity && c.Type == PotionToCardType[potion2]).WithFlags(CardCreationFlags.NoRarityModification);
