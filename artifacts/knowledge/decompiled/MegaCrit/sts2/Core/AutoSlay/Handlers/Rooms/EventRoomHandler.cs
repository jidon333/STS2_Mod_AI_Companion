...
public class EventRoomHandler : IRoomHandler, IHandler
{
	...
	{
		Node eventRoom2 = eventRoom;
		NAncientEventLayout nAncientEventLayout = UiHelper.FindFirst<NAncientEventLayout>(eventRoom2);
		if (nAncientEventLayout != null)
		{
			await HandleAncientEventDialogue(nAncientEventLayout, ct);
			return false;
		}
		NFakeMerchant nFakeMerchant = UiHelper.FindFirst<NFakeMerchant>(eventRoom2);
		...
		await WaitHelper.Until(delegate
		{
			...
			if (waitCycles % 50 == 0)
			{
				int childCount = eventRoom2.GetChildCount();
			...
			return UiHelper.FindAll<NEventOptionButton>(eventRoom2).Count > 0;
	...
	{
		NFakeMerchant fakeMerchant2 = fakeMerchant;
		AutoSlayLog.Action("Handling FakeMerchant event");
		NProceedButton proceedButton = null;
		await WaitHelper.Until(delegate
		{
			proceedButton = UiHelper.FindFirst<NProceedButton>(fakeMerchant2);
	...
	{
		NAncientEventLayout ancientLayout2 = ancientLayout;
		AutoSlayLog.Info("Detected Ancient event, clicking through dialogue");
		int clicks = 0;
		while (clicks < 50)
		{
			ct.ThrowIfCancellationRequested();
			if (!GodotObject.IsInstanceValid(ancientLayout2))
			{
				break;
			}
			List<NEventOptionButton> list = (from b in UiHelper.FindAll<NEventOptionButton>(ancientLayout2)
			...
			NButton nodeOrNull = ancientLayout2.GetNodeOrNull<NButton>("%DialogueHitbox");
		...
		await WaitHelper.Until(() => UiHelper.FindAll<NEventOptionButton>(ancientLayout2).Any((NEventOptionButton b) => b.IsEnabled && !b.Option.IsLocked), ct, TimeSpan.FromSeconds(10L), "Ancient event options did not become available after dialogue");
	}
}
