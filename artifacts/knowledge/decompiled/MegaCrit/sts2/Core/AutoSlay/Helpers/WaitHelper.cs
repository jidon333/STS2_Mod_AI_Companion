...
public static class WaitHelper
{
	...
	{
		Node root2 = root;
		string nodePath2 = nodePath;
		try
		{
			await Until(() => root2.HasNode(nodePath2), ct, timeout, $"Node {nodePath2} of type {typeof(T).Name} not found");
		}
		catch (TimeoutException)
		{
			DumpSceneTreeContext(root2, nodePath2, "not found");
			throw;
		}
		T node = root2.GetNode<T>(nodePath2);
		Control control = node as Control;
		if (control != null)
		{
			try
			{
				await Until(() => control.Visible, ct, timeout, "Node " + nodePath2 + " not visible");
			}
			catch (TimeoutException)
			{
				DumpSceneTreeContext(root2, nodePath2, "not visible");
				throw;
			}
			...
			if (nButton != null)
			{
				try
				{
					await Until(() => nButton.IsEnabled, ct, timeout, "Button " + nodePath2 + " not enabled");
				}
				catch (TimeoutException)
				{
					DumpSceneTreeContext(root2, nodePath2, "not enabled");
					throw;
				}
			}
		}
