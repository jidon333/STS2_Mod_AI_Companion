...
> Status: Snapshot
> Source of truth: No
> This is a point-in-time UI flow draft. Current product direction lives in [ROADMAP.md](../../ROADMAP.md) and [PROJECT_STATUS.md](../../current/PROJECT_STATUS.md).

이 문서는 사용자가 실제로 보게 될 화면과 사용 흐름을 설명합니다.

중요:

- 이 문서는 `advisor mode` 기준 사용자 흐름입니다.
- harness mode는 별도 테스트 운영 모드이며, WPF는 그 자체가 harness 제어면이 아닙니다.
...
## 11. harness mode와의 관계

이 UI는 최종 제품 표면이므로 다음 원칙을 유지합니다.

- 사용자 대신 입력하지 않음
- game action executor를 직접 가지지 않음
- read-only observer / advisor contract 유지

harness mode는 같은 foundation을 공유하지만 별도 layer에서 동작합니다.

- shared foundation
  - `CompanionState`
  - knowledge / session / artifact vocabulary
- advisor mode
  - 현재 WPF 화면
- harness mode
  - action / scenario / recovery / evaluation

즉 WPF는 dual-mode 구조 안에서 `production advisor surface`로만 남습니다.
