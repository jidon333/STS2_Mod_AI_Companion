...
- `COMBAT-03`, `COMBAT-07`, `REWARD-04`, `MAP-04` coverage row는 current `main`에서 green

이번 세션 목표

1. old `Program.cs` monolith 전제를 버리고 current owner 파일 기준으로 시작
2. cleanup-complete baseline reopen 금지
3. semantic follow-up이 필요하면 low-priority coverage frontier (`EVENT-05`, `REWARD-10`)만 좁게 다룰 것
