...
즉 지금 남은 핵심은 unrelated self-test red가 아니라, **anti-drift recovery 뒤 확보한 `live9` baseline을 유지한 채 child-screen smoothness gap을 좁히는 것**과 그 다음 first blocker 1개만 정확히 분류하는 것입니다.
...
이제 남은 건 old screenshot/broad-fallback 철학으로 되돌아가지 않은 채 `live9` fresh live baseline 위에서
`deck-remove` / reward card child-screen explicit owner/export를 보강하고,
