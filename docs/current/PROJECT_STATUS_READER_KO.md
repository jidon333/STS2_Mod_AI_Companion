...
지금은 “하네스 구조를 사람이 읽을 수 있게 정리하는 작업”, “대표 hot path를 observer-first로 되돌리는 작업”, “observer provenance / runner / noncombat cleanup”이 큰 틀에서 끝난 상태입니다.
...
다만 `Sts2GuiSmokeHarness -- self-test`는 현재 unrelated known red 1건이 있습니다.

- `WaitPostMapNodeRoom should reopen reward handling when the destination room is rewards.`
- 즉, 구조/전투 blocker는 많이 닫혔지만 self-test 전체가 완전 green인 상태는 아닙니다.
...
즉 지금 남은 핵심은 combat blocker보다, unrelated `WaitPostMapNodeRoom` self-test red와 일부 low-priority coverage frontier입니다.
...
```text
하네스 구조 정리, reward aftermath mixed-state cleanup, common hot path speed recovery, published-first observer provenance migration, post-refactor cleanup program은 끝났고,
combat stale-end-turn / target plateau family도 current `main`에서 닫혔고, explicit shop foreground 위의 stale reward misroute plateau도 current `main`에서 회복 경로가 고정됐다.
이제 남은 건 unrelated `WaitPostMapNodeRoom` self-test red와 일부 low-priority coverage frontier를
