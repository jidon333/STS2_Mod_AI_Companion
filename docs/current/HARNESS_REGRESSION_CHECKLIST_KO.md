...
| State ID | Authority | Foreground owner / lane | Allowed actions | Forbidden actions | Release / handoff | Mixed residue rule | Current evidence |
|---|---|---|---|---|---|---|---|
| `MAP-01` Pure map foreground | explicit map current active screen, `activeScreenType=NMapScreen`, reachable map node | Map / map-node | exported reachable node, map back, wait | event/reward/shop action reuse, screenshot-derived node guess | node click -> `WaitPostMapNodeRoom` or `WaitMap` | stale room labels are residue | `Program.AllowedActions.NonCombat.cs`, `Observer/MapForegroundReconciliation.cs` |
...
| `COMBAT-02` Card selection | selected-card truth, exported/runtime hand truth, or explicit slot action | Combat / select-card | legal card select hotkey or click, wait | direct target click before legal selection, screenshot-only slot guess | release to targeting or resolve no-target action only when runtime/exported ownership stays explicit | screenshot-only slot drift and stale history shadow are disallowed | `AutoDecisionProvider.CombatDecisions.cs`, `Analysis/CombatRuntimeStateSupport.cs`, `Program.SelfTests.CombatContracts.*.cs` |
| `COMBAT-03` Target selection | explicit enemy target nodes/body rects, selected card requires target | Combat / target | click exported enemy target | blind fixed anchor click, end turn while target unresolved | resolve action or cancel selection | target bounds beat screenshot anchor, runtime `combatTargetSummary` raw fact도 explicit target authority로 쓴다 | `Analysis/CombatMicroStageSupport.cs`, `Analysis/CombatPostActionObservationSupport.cs`, `Analysis/CombatTargetabilitySupport.cs`, `Program.SelfTests.CombatContracts.TargetSelection.cs` |
...
| `COMBAT-05` End turn pre-ack | end turn fired, no turn-transition acknowledgement yet | Combat / end-turn barrier | wait only | reopen player action, target, or map fallback | release to acknowledged transit or bounded failure | no timer-based actuation, unresolved combat lane에서는 end turn 자체를 열지 않는다 | [PROJECT_STATUS.md](/mnt/c/users/jidon/source/repos/sts2_mod_ai_companion/docs/current/PROJECT_STATUS.md), [combat-target-summary-20260330-live2](/mnt/c/Users/jidon/source/repos/STS2_Mod_AI_Companion/artifacts/gui-smoke/combat-target-summary-20260330-live2) |
| `COMBAT-06` End turn acknowledged transit | transition ack present, enemy-turn/closed-play-phase band in progress | Combat / transit wait | wait only | premature plateau classification, reissuing end turn | release on next-round reopen, or bounded failure if frozen | authoritative turn progress must suppress plateau, generic observer delta는 combat post-action proceed 조건이 아니다 | `Program.PhaseLoopRouting.cs`, `Analysis/CombatPostActionObservationSupport.cs`, `Program.SelfTests.CombatContracts.ParityAndBarriers.cs` |
...
| `COMBAT-11` Combat release under noncombat foreground | `HandleCombat` phase but canonical `event/reward/shop/map` owner와 explicit surface가 서고 combat barrier는 residue-only | Released combat / target room lane | branch to `HandleEvent`, `HandleRewards`, `HandleShop`, `ChooseFirstNode`, `WaitMap`; family-specific wait/abort | keeping combat barrier as primary authority under explicit noncombat foreground; generic `decision-wait-plateau` | explicit release evidence가 canonical하게 서면 combat ownership을 release하고 target branch로 handoff | `screen != combat` alone is not enough; explicit noncombat owner + surface만 release authority다 | `AutoDecisionProvider.CombatDecisions.cs`, `Analysis/CombatBarrierSupport.cs`, `Program.SelfTests.CombatContracts.ParityAndBarriers.cs`, `Program.SelfTests.StallSentinel.cs` |
...
| `REST-03` Post-rest release pending over map overlay | `screen=rest-site`, recent selection success, no explicit rest/proceed surface yet, map overlay/exported node residue present | RestSite / release-pending | wait, rest-site proceed once explicit | exported map node click before true map foreground | proceed -> room release, true map foreground -> map routing | `choiceExtractorPath=map`, map overlay, exported node are diagnostics-only residue while rest-site release-pending stays canonical | `AutoDecisionProvider.NonCombatSceneState.cs`, `Program.SelfTests.NonCombatForegroundOwnership.cs`, `Program.SelfTests.PhaseRouting.EnterRunAndPostNode.cs` |

## Rewards

| State ID | Authority | Foreground owner / lane | Allowed actions | Forbidden actions | Release / handoff | Mixed residue rule | Current evidence |
|---|---|---|---|---|---|---|---|
| `REWARD-01` Explicit reward item claim | reward foreground active, explicit reward node/choice visible | Rewards / claim | claim reward item before proceed/map | map fallback or reward skip before claimable item if claim exists | claim -> remain in `HandleRewards` while more reward affordance exists | reward-owned explicit `reward-type:*Reward` row는 inspect preview보다 강하고, map background may be visible but cannot steal owner | `AutoDecisionProvider.NonCombatDecisions.cs`, `Program.SelfTests.NonCombatDecisionContracts.RewardContracts.cs` |
| `REWARD-02` Reward card / colorless | reward card row or colorless reward choice state visible | Rewards / card-choice | reward card choice, colorless card choice, reward skip/proceed, back when explicit | visible map advance while reward card visible | card pick -> remain in reward or move to next explicit reward state | inspect previews are not claim actions, and bare generic `card` residue without reward-backed evidence is not reward authority | `AutoDecisionProvider.NonCombatDecisions.cs`, `Program.SelfTests.NonCombatDecisionContracts.RewardContracts.cs` |
| `REWARD-03` Reward skip/proceed | explicit reward proceed/skip visible, no higher-priority claim item | Rewards / skip-proceed | reward skip or proceed | proceed while explicit claim item or explicit reward card row still exists; map fallback before reward affordance disappears | skip/proceed -> release pending then map handoff | map background visible is residue while skip/proceed still foreground active | `AutoDecisionProvider.NonCombatDecisions.cs`, `Program.SelfTests.NonCombatDecisionContracts.RewardContracts.cs` |
...
| `REWARD-08` Reward aftermath card progression | reward foreground owner explicit, lingering event aftermath may remain, explicit `CardReward` row는 있지만 subtype/card-selection publish가 아직 안 열림 | Rewards / card-choice wait | reward card choice or family-specific wait | generic same-action-stall, generic screenshot drift, proceed before claim/card-row exhaustion | card row click -> reward subtype/card-selection publish, 또는 `reward-aftermath-card-progression-stall`로 bounded failure | `choiceExtractorPath=event` 같은 aftermath residue는 diagnostics일 뿐 reward owner나 proceed precedence를 뒤집지 못한다 | `AutoDecisionProvider.NonCombatDecisions.cs`, `Program.Runner.AttemptDecisionFlow.cs`, `Program.SelfTests.NonCombatDecisionContracts.RewardContracts.cs` |
...
|---|---|---|---|---|
| P1 | Reward aftermath card progression | partial | explicit `CardReward` row는 reward-family wait/stall로 분리됐지만 fresh live root에서 repeatability evidence가 아직 얇다 | `reward-aftermath-card-progression-stall` live evidence와 subtype publish progression을 더 고정 |
| P1 | Combat release under noncombat foreground | partial | short validation roots에서는 `combat-release-failure-under-noncombat-foreground`, long-tail에서는 `combat-barrier-step-budget-exhausted`가 남을 수 있다 | release evidence와 subtype classification을 더 고정하고 live repeatability를 확보 |
| P1 | Live review video usability | partial | ffmpeg metadata는 남지만 `gdigrab` review video가 black frame이라 screenshot/request artifact에 계속 의존한다 | review video quality를 gameplay blocker와 분리해 별도 validation family로 정리 |
| P2 | Event reward substate evidence | low-priority noncombat coverage frontier | event -> reward delegated lane의 fresh live evidence가 아직 얇다 | reward/event mixed live root를 current matrix에 추가 |
| P2 | Reward-map loop sentinel evidence | fallback sentinel evidence only | sentinel exists but current-main evidence가 얇다 | sentinel을 primary flow로 승격하지 말고 evidence만 보강 |
