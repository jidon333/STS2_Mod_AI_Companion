...
|---|---|---|---|---|
| P1 | Reward card click progression | partial | explicit `CardReward` row가 same-action-stall로 새 front blocker가 될 수 있다 | click progression drift를 reward owner/action/release contract 안에서 좁게 정리 |
| P1 | Combat barrier ownership release | partial | short validation roots에서 `combat-barrier-step-budget-exhausted` / `combat-barrier-wait-plateau`가 남는다 | barrier ownership release와 noncombat handoff evidence를 family-specific하게 더 고정 |
