...
- `CodexCliClient`
  ...
  - JSON event stream 기반 session id 추적
...
- manual advice 실행 시 `sessionId` 캡처
- `codex-session.json` 저장

### 아직 닫히지 않은 것

- automatic advice의 최신 gameplay 재실증
- gameplay trigger에서의 run-scoped session resume

현재 최신 manual advice 기준으로는 `sessionId` 캡처와 `codex-session.json` 생성이 성공했습니다. 따라서 지금 상태는 “Codex와 대화해 조언을 받고 manual path에서 런 세션을 기록하는 것”은 성공, “실제 gameplay trigger에서도 런 단위 세션을 계속 이어 붙이는 것”은 미완료입니다.
...
2. gameplay-triggered session id capture / resume 경로를 닫기
