...
- combat enemy-targeting은 fixed anchor 위주에서 live hitbox/export 기반으로 상당히 개선되었다.
- reward-map recovery는 실제로 전진했고, reward/map layered state와 recovery window가 도입되었다.
- 최신 핵심 병목은 reward/map보다 더 앞선 `event 뒤 map overlay mixed-state` 처리다.
- `replay-step`, `replay-test`, golden scene 회귀 검증, startup tracing이 추가되어 운영 진단과 오프라인 재현성이 강화되었다.

## 0-1. 현재 아키텍처 상태 요약

2026년 3월 16일 기준 현재 상태를 한 문장으로 요약하면 다음과 같다.

`하네스는 screenshot-first를 유지한 채 observer/export를 보조 입력으로 점진 승격하고 있으며, combat과 reward-map recovery는 전진했지만 최신 blocker는 event/map overlay mixed-state와 loop terminalization 쪽으로 이동했다.`

### 현재 진척도 스냅샷

| 항목 | 수치 | 해석 |
|---|---:|---|
| Overall | 76% | 전체 구조는 유효하지만 mixed-state와 loop 문제가 아직 남아 있음 |
| Evidence | 90% | artifact 계약, trust/milestone 증거 체계는 강함 |
| Observer | 77% | export와 validation 입력은 좋아졌지만 mixed-state authority는 더 필요함 |
| Actuator | 66% | combat과 reward-map은 전진했지만 event/map overlay 전환이 약함 |
| Loop | 63% | sentinel과 terminalization은 개선됐지만 mixed-state 반복 루프는 계속 보강 중 |
| Ops | 86% | inspect-session, supervision, trust gate, 운영 가드레일은 비교적 안정적 |

### 이 수치가 말하는 것

- 지금 가장 강한 부분은 `Evidence`와 `Ops`다.
- 지금 가장 약한 부분은 `Actuator`와 `Loop`다.
- 즉 현재 문제는 "무엇이 일어났는지 모른다"보다 "mixed-state에서 다음 행동을 안정적으로 고르고, 잘못된 반복을 빨리 끝내기 어렵다"에 더 가깝다.
- 최근 커밋 기준으로는 `startup-trace.ndjson`, `startup-summary.json`, `replay-step`, `replay-test`, golden scene suite가 `Ops`와 `Evidence`의 강도를 더 끌어올린 변화다.

이 문서를 읽기 쉽게 만들기 위해, 이번 버전은 아래 순서로 설명한다.

1. 큰 그림
2. 쉬운 용어 설명
3. 실제 세션이 어떻게 흘러가는지
4. 그 다음에야 artifact와 상태 모델
...
- `docs/development/05-harness/README.md`
- `docs/development/05-harness/HARNESS_MODE.md`
- `docs/development/05-harness/STS2_HARNESS_REVIEW.md`
...
3. 게임을 실행하고 `attempt 0001`을 시작한다.
4. 매 step마다 스크린샷과 observer 복사본, progress 로그를 남긴다.
5. attempt가 끝나면 terminal event를 기록한다.
6. runner가 다음 attempt를 다시 시작하려고 하면 restart event를 기록한다.
7. `attempt 0002`의 첫 스크린샷이 실제로 생기면 restart progression 증거 체인이 닫힌다.
8. 이때 trust gate까지 valid였다면 supervisor는 long-run milestone을 `done`으로 올릴 수 있다.
...
| `restart-events.ndjson` | terminal, restart, next-attempt-start 같은 순서 이벤트 기록 | runner | supervisor, stall sentinel | restart progression 증거 체인 | runner 외부 주체의 restart 구조는 아직 없음 | `이미 구현됨` |
| `attempt-index.ndjson` | 각 attempt의 요약 인덱스 | runner | supervisor, stall sentinel, 분석자 | terminal cause, failure class, trustStateAtStart 분류 | attempt가 중간에 죽으면 원시 truth는 restart-events 쪽이 더 강함 | `이미 구현됨` |
| `supervisor-state.json` | trust/milestone/health 판정 결과 | supervisor | runner, 인간 조율자, 테스트 에이전트 | 세션 상태 요약과 blocker/evidence 확인 | 상주 감시 서비스의 push state는 아님 | `이미 구현됨` |
...
이 전략이 계속 필요한 이유는 최신 blocker의 성격 때문이다. 지금 문제의 초점은 "이벤트 옵션 텍스트를 전혀 못 읽는다"보다는, "event foreground 위에 섞여 보이는 map arrow를 foreground actionable target처럼 승격한다"로 이동했다.

### Transient vs Authoritative

| 분류 | 예시 | 왜 그렇게 보는가 |
|---|---|---|
| authoritative에 가까운 것 | `goal-contract.json`, `prevalidation.json`, `restart-events.ndjson`, `attempt-index.ndjson`, `steps/*.screen.png`, decompiled-backed confirmation | 나중에 다시 읽어도 같은 판정을 재현하기 쉬움 |
| transient하거나 보조적인 것 | 단일 `state.latest.json`, 콘솔 commentary, 순간적인 observer choice set | 일시적이거나 재검증 근거로 약함 |

## 11. 최근 실제 병목 이동

이 섹션은 "예전에 어디가 문제였고, 지금은 어디가 더 큰 병목인가"를 보여 주기 위한 진단 섹션이다.
