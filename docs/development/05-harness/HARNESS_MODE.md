...
## Update (2026-03-13): observer strategy refinement

- harness observer는 `event + polling mixed observer`로 본다.
- polling은 continuous state, reconciliation, watchdog 역할을 계속 맡는다.
- 다만 `scene transition`, `screen ready`, `lifecycle boundary` 판단은 decompiled-backed transition-oriented hook를 우선 검토한다.
- transient polled scene만으로 `scene-ready`를 단정하지 않는다.
- 다음 observer 개선 작업은 먼저 `artifacts/knowledge/decompiled`에서 main menu, singleplayer submenu, character select 흐름을 읽는 것부터 시작한다.
- `dispatch_node`는 authoritative scene stabilization이 확보되기 전까지 계속 닫아 둔다.
- 자세한 기준은 `docs/development/05-harness/DECOMPILED_SOURCE_FIRST_OBSERVER_STRATEGY.md`를 따른다.