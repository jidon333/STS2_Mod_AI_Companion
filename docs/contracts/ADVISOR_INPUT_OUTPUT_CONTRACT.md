...
현재 목표는 `reward/event/shop` compact advisor path를 유지하면서, `background-only strategy principles`와 `최종 / 보수적 / 공격적` additive advice view를 기존 `AdviceInputPack` / `AdviceResponse` 위에 안전하게 연결하는 것이다.
