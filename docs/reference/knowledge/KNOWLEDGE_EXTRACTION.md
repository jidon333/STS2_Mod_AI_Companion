...
> Status: Reference
> Source of truth: No
> Use this for knowledge pipeline background, not for current blocker or live runtime status.
...
- 현재는 historical companion `live-mirror`와 현재 live root를 함께 읽어 observed data를 누적합니다.
- 최신 기준으로 `observed-merge`는 비어 있지 않으며, reward / event / relic / potion / card-added 관찰값이 실제로 합쳐지고 있습니다.
- 이 단계는 exporter coverage가 넓어질수록 더 중요해집니다.
...
현재 최신 `inspect-static-knowledge` 기준 최종 catalog counts:
...
- 카드 설명에는 일부 unresolved placeholder가 남습니다.
  - 예: `{PlayMax:diff()}`
- `observed-merge`는 이미 historical live-mirror 기준으로 일부 채워졌지만, shop/rest/combat 선택지까지 풍부하게 채우려면 gameplay smoke coverage가 더 넓어져야 합니다.
