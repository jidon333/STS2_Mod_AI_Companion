...
public sealed class KnowledgeCatalogService
{
    ...
    public StaticKnowledgeCatalog ReloadIfChanged()
    {
        ...
        _catalog = SanitizeCatalog(
            JsonSerializer.Deserialize<StaticKnowledgeCatalog>(stream, ConfigurationLoader.JsonOptions)
    ...
    private static StaticKnowledgeCatalog SanitizeCatalog(StaticKnowledgeCatalog catalog)
    {
        return new StaticKnowledgeCatalog(
            catalog.GeneratedAt,
            catalog.Metadata,
            SanitizeEntries(catalog.Cards),
            SanitizeEntries(catalog.Relics),
            SanitizeEntries(catalog.Potions),
            SanitizeEntries(catalog.Events),
            SanitizeEntries(catalog.Shops),
            SanitizeEntries(catalog.Rewards),
            SanitizeEntries(catalog.Keywords));
    }

    private static IReadOnlyList<StaticKnowledgeEntry> SanitizeEntries(IReadOnlyList<StaticKnowledgeEntry>? entries)
    {
        if (entries is null)
        {
        ...
        return entries.Select(entry => entry with
        {
            ...
            Attributes = entry.Attributes ?? new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase),
            Options = entry.Options ?? Array.Empty<StaticKnowledgeOption>(),
        }).ToArray();
    }
